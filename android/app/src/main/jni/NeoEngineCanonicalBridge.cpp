...
Java_com_neoengine_core_NeoEngineCanonicalBridge_nativeLifecycleEvent(JNIEnv*, jclass, jint eventCode, jfloat deltaSeconds) {
    bool accepted = false;
    if (eventCode == 3 && (!std::isfinite(deltaSeconds) || deltaSeconds < 0.0F || deltaSeconds > 0.25F)) return JNI_FALSE;
    switch (eventCode) {
        case 0: accepted = g_lifecycle.Initialize(); if (accepted) g_rendererReady = g_renderer.Initialize(256U, 256U); accepted = accepted && g_rendererReady; break;
        ...
        case 4: g_rendererReady = false; g_renderer = NeoEngine::SoftwareRenderer{}; accepted = g_lifecycle.Shutdown(); break;
