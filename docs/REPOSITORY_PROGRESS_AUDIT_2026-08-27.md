...
| Mandatory release gates | **5/12 = 41,7% lulus pada scoped Farm evidence** | R1, R2, R3, R4, dan R5 lulus untuk canonical Farm scope; R6–R12 belum lulus formal dan template lain memerlukan evidence independen. |
...
> **Kesimpulan paling jujur:** repo sudah sekitar **82% menyelesaikan backlog engineering yang tercatat**, tetapi baru sekitar **51% menuju standar release end-to-end** menurut estimasi evidence parsial, dan **5 dari 12 gate** lulus hanya pada scope Farm yang dibatasi: R1 tool, R2 vertical slice, R3 renderer, R4 input/audio, dan R5 authoritative. R6–R12 belum lulus formal global; ini tetap bukan release readiness.
...
| R3 Asset and renderer path | 100% scoped Farm renderer | Passed for canonical Farm scope | Asset/texture/mesh/material bounded proofs, Farm manifest, and Vulkan textured present connect a canonical NeoRuntime Farm/HUD framebuffer to swapchain presentation; Release/ASAN local smokes and remote run `33137066510` on `a865577` pass. Physical-GPU/device-loss/resize, streaming, performance, and Android GPU delivery remain outside this scoped pass. |
| R4 Input, audio, accessibility | 100% scoped Farm lifecycle | Passed for canonical input/audio lifecycle scope | Canonical SDL/InputState/UI and PCM/SDL reset/reinitialize lifecycle now have Release/ASAN dummy-device evidence; remote run `33137030685` on `f80898e` pass. Physical output, Android audio focus, touch/controller devices, localization/scaling, IME, accessibility, and full UX acceptance remain outside this scoped pass. |
| R5 Authoritative multiplayer | 100% scoped Farm authority | Passed for canonical Farm authoritative scope | Farm authoritative host now has bounded localhost TCP session loopback with subject/session rejection, replay stability, and opt-in two-connection reconnect; Release/ASAN local smokes and remote run `33137268700` on `9458d4e` pass. TLS, durable session, public transport, and multi-client load remain outside this scoped pass. |
