...
`FarmRuntimeSession::RouteHudPointer` and `RouteHudKeyboard` delegate only to `FarmActionPanelController`, which changes the session's existing `FarmPlayerInputBridge` selected action. The UI never calls `FarmWorldTool`. A later `FarmRuntimeSession::Frame` with canonical `farm_interact` input remains the only path that executes Till, Plant, Water, or Harvest against the Farm world.
