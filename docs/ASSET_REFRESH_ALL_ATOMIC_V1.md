...
`AssetRefreshExecutor::ExecuteAllAtomic` copies texture, mesh, material, scene-mesh, sprite, and prefab stores. It runs dependency-aware resource actions, then sprite actions, then prefab actions on that shared candidate set. Only total success commits all stores and plan-order receipt vectors. Every committed preflight and execution receipt copies the corresponding diagnostics `expectedHash`, so a caller can audit the approved registry version without consulting mutable registry state. A later malformed prefab therefore rolls back earlier candidate texture and sprite refreshes while retaining prior committed receipts.
