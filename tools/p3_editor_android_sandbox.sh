#!/usr/bin/env bash
set -euo pipefail
BUILD_DIR="${1:-Build/p3_complete}"
BUILD_TYPE="${P3_BUILD_TYPE:-Release}"
...
for target in "${TARGETS[@]}"; do
  if SDL_VIDEODRIVER="${SDL_VIDEODRIVER:-dummy}" SDL_AUDIODRIVER="${SDL_AUDIODRIVER:-dummy}" "$BUILD_DIR/$target" >>"$REPORT_DIR/execution.log" 2>&1; then
    printf "%s\tPASS\n" "$target" | tee -a "$REPORT_DIR/execution.txt"; pass=$((pass+1))
  else
    rc=$?; printf "%s\tFAIL\texit=%d\n" "$target" "$rc" | tee -a "$REPORT_DIR/execution.txt"; fail=$((fail+1))
  fi
done
bash tools/android_lifecycle_bridge_source_smoke.sh >"$REPORT_DIR/android_lifecycle_bridge.log" 2>&1
printf "P3 editor/android complete coverage\ntargets=%d\npass=%d\nfail=%d\nskip=0\n" "${#TARGETS[@]}" "$pass" "$fail" | tee "$REPORT_DIR/summary.txt"
git fetch --unshallow origin main 2>/dev/null || git fetch --no-tags origin main
...
if [[ -s "$REPORT_DIR/static_markers.txt" ]]; then cat "$REPORT_DIR/static_markers.txt"; exit 1; fi
(( fail == 0 ))
