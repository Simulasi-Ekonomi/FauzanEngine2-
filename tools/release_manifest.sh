...
if [[ "$OUT" == * echo "RELEASE_MANIFEST_FAIL output_symlink" >&2; exit 2; fi
TMP="$(mktemp)"
trap 'rm -f "$TMP"' EXIT
...
\n'* || "$OUT" == * echo "RELEASE_MANIFEST_FAIL output_symlink" >&2; exit 2; fi
TMP="$(mktemp)"
trap 'rm -f "$TMP"' EXIT
...
printf 'RELEASE_MANIFEST_OK commit=%s files=%s output=%s\n' \
  "$(git rev-parse --short HEAD)" "$(wc -l < "$TMP")" "$OUT"
\r'* ]]; then echo "RELEASE_MANIFEST_FAIL unsafe_output" >&2; exit 2; fi
