...
if [[ ! -f "$OUT" || -L "$OUT" || ! -s "$OUT" ]]; then echo "RELEASE_MANIFEST_FAIL output_invalid" >&2; exit 2; fi
printf 'RELEASE_MANIFEST_OK commit=%s files=%s output=%s\n' "$(git rev-parse --short HEAD)" "$(wc -l < "$TMP")" "$OUT"
\n'* || "$OUT" == * || "$OUT" == ../* || "$OUT" == */../* ]]; then echo "RELEASE_MANIFEST_FAIL unsafe_output" >&2; exit 2; fi
if [[ -L "$OUT" ]]; then echo "RELEASE_MANIFEST_FAIL output_symlink" >&2; exit 2; fi
TMP="$(mktemp)"
trap 'rm -f "$TMP"' EXIT
git ls-files -z ':!*.keystore' ':!*.jks' ':!*.p12' ':!build/' ':!out/' ':!dist/' ':!.git/' |
...
if [[ ! -f "$OUT" || -L "$OUT" || ! -s "$OUT" ]]; then echo "RELEASE_MANIFEST_FAIL output_invalid" >&2; exit 2; fi
printf 'RELEASE_MANIFEST_OK commit=%s files=%s output=%s\n' "$(git rev-parse --short HEAD)" "$(wc -l < "$TMP")" "$OUT"
\r'* || "$OUT" == ../* || "$OUT" == */../* ]]; then echo "RELEASE_MANIFEST_FAIL unsafe_output" >&2; exit 2; fi
if [[ -L "$OUT" ]]; then echo "RELEASE_MANIFEST_FAIL output_symlink" >&2; exit 2; fi
TMP="$(mktemp)"
trap 'rm -f "$TMP"' EXIT
git ls-files -z ':!*.keystore' ':!*.jks' ':!*.p12' ':!build/' ':!out/' ':!dist/' ':!.git/' |
