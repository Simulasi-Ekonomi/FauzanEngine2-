...
[[ -z "$(git status --porcelain=v1 --untracked-files=all)" ]] || { echo "P4_RELEASE_CERTIFICATION_FAIL dirty_worktree" >&2; exit 1; }
...
for tool in git python3 sha256sum bash; do command -v "$tool" >/dev/null 2>&1 || fail "missing_tool_$tool"; done
...
\n'* && "$path" != *
    ...
    [[ "$path" != */ ]] || fail "directory_artifact_path"
    [[ -f "$path" && ! -L "$path" && -s "$path" ]] || fail "artifact_not_regular_file"
    [[ "$path" != "$ROOT"/* || -f "$path" ]] || fail "artifact_path_resolution"
done
