    ...
    try:
        with zipfile.ZipFile(artifact) as archive:
            ...
            for info in archive.infolist():
                name=info.filename
                if "@" in name and name.startswith("@"): raise SystemExit("P4_ARTIFACT_GATE_FAIL unsafe_zip_path")
                if "\\" in name or name.startswith("/") or name.startswith("./") or ".." in Path(name).parts: raise SystemExit("P4_ARTIFACT_GATE_FAIL unsafe_zip_path")
                if len(name)>1024: raise SystemExit("P4_ARTIFACT_GATE_FAIL zip_name_too_long")
                if info.flag_bits & 0x1 or info.flag_bits & ((1<<5)|(1<<6)|(1<<13)|(1<<14)|(1<<15)): raise SystemExit("P4_ARTIFACT_GATE_FAIL unsafe_zip_flags")
                if info.file_size>MAX_ENTRY_SIZE: raise SystemExit("P4_ARTIFACT_GATE_FAIL oversized_zip_entry")
                ...
                if (info.external_attr>>16)&0o170000==0o120000: raise SystemExit("P4_ARTIFACT_GATE_FAIL symlink_zip_entry")
