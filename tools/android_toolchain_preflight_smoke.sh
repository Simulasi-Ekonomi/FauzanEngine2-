...
grep -Fq 'ANDROID_PACKAGING_PREFLIGHT_READY source_alignment=canonical_subset api_target=36 build_not_run=1' <<<"$output"
grep -Fq 'ANDROID_APK_AAB_NOT_BUILT' <<<"$output"
echo 'ANDROID_TOOLCHAIN_PREFLIGHT_SMOKE_OK toolchain=1 sourceAlignment=1 packageNotBuiltByPreflight=1'
