...
import { WorldGeneratorPanel } from '../WorldGenerator/WorldGeneratorPanel';
...
export function MenuBar() {
  ...
  const handleWorldGenerate = (config: any) => {
    setWorldGenOpen(false);
    store.addSystemMessage(`[WorldGen] Generating ${config.sizeKm}x${config.sizeKm} km world with seed ${config.seed}...`);
    // Panggil native bridge untuk memulai streaming dunia
    if (typeof (window as any).NeoEngineBridge !== 'undefined') {
      (window as any).NeoEngineBridge.startWorldStreaming(config.seed, config.sizeKm);
    }
    // Simpan konfigurasi ke store untuk referensi
    store.addSystemMessage(`[WorldGen] Trees: ${config.treeDensity}/km², Resources: ${config.resourceDensity}/km², Cities: ${config.cityCount}`);
  };
  ...
  const menuDefs: Record<string, MenuItem[]> = {
    ...
    Tools: [
      ...
      { separator: true },
      { label: '🌍 World Generator...', action: () => setWorldGenOpen(true) },
      { separator: true },
  ...
  return (
    <>
      ...
      <WorldGeneratorPanel
        ...
        onGenerate={handleWorldGenerate}
      />
    </>
  );
}
