...
void AndroidPlatform::PumpEvents() {
    // Event delivery is owned by the Android glue layer through Inject*Event.
    // Pumping must never discard queued input before the gameplay/input systems
    // have had a chance to consume it.
    const uint64_t now = GetTimeNano();
    const uint64_t previous = m_LastPumpNano == 0 ? now : m_LastPumpNano;
    m_LastPumpNano = now;
    const float deltaSeconds = static_cast<float>(now - previous) * 1.0e-9f;
    if (m_MainLoop) m_MainLoop(deltaSeconds);
}
