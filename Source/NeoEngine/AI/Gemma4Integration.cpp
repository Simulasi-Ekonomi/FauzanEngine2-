...
static std::string execCommand(const std::string& command){std::string result;FILE* pipe=popen(command.c_str(),"r");if(!pipe)return result;char buffer[1024];while(fgets(buffer,sizeof(buffer),pipe)!=nullptr)result+=buffer;pclose(pipe);return result;}
Gemma4Response Gemma4Integration::GenerateText(const std::string& prompt,int maxLength){Gemma4Response resp{};if(!ready||prompt.empty()||maxLength<=0||maxLength>131072)return resp;const std::string command="gemma --model "+ShellQuote(modelFilePath)+" --prompt "+ShellQuote(prompt)+" --max_tokens "+std::to_string(maxLength);const std::string output=execCommand(command);if(output.empty())return resp;resp.generatedText=output;resp.confidence=0.9f;resp.tokensUsed=static_cast<int>(std::min<size_t>(output.size()/4U,static_cast<size_t>(std::numeric_limits<int>::max())));return resp;}
...
void Gemma4Integration::SetModelSize(Gemma4ModelSize size){if(!ready)modelSize=size;}
}