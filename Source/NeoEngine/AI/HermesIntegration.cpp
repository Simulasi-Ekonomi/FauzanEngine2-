...
namespace NeoEngine {
static size_t WriteCallback(void* contents,size_t size,size_t nmemb,std::string* output){if(output==nullptr||contents==nullptr||(nmemb!=0U&&size>std::numeric_limits<size_t>::max()/nmemb))return 0U;const size_t total=size*nmemb;if(total>16U*1024U*1024U||output->size()>16U*1024U*1024U-total)return 0U;output->append(static_cast<const char*>(contents),total);return total;}
