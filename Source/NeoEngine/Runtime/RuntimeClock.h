...
enum class RuntimeClockError : uint8_t { None, InvalidConfiguration, InvalidDelta, InvalidScale, NotInitialized, FixedStepOverrun, Overflow };
...
struct RuntimeClockSnapshot { float unscaledDeltaSeconds = 0.0F; float scaledDeltaSeconds = 0.0F; double unscaledTimeSeconds = 0.0; double scaledTimeSeconds = 0.0; uint64_t frameCount = 0; uint64_t fixedStepCount = 0; uint8_t pendingFixedSteps = 0; uint64_t droppedFixedStepCount = 0; float droppedFixedSeconds = 0.0F; bool paused = false; float timeScale = 1.0F; };
