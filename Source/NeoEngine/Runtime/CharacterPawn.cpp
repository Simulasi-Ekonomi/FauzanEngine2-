...
bool ValidCharacterSnapshot(const CharacterPawnSnapshot& snapshot, SceneEntity actor, const CharacterPawnConfig& config, const CharacterAnimationGraph& currentAnimation) {
    ...
    if (snapshot.actor != actor || (snapshot.authority != CharacterMovementAuthority::None && snapshot.authority != CharacterMovementAuthority::KinematicRoute && snapshot.authority != CharacterMovementAuthority::SkeletalRoot) || (snapshot.rootMotionMode != CharacterRootMotionMode::Kinematic && snapshot.rootMotionMode != CharacterRootMotionMode::SkeletalRoot) || !ValidRootMotion(snapshot.velocity) || (snapshot.grounded && std::abs(snapshot.velocity.y) > 0.0001F) || !Finite(inputMagnitudeSquared) || inputMagnitudeSquared > config.maxPlanarInput * config.maxPlanarInput || !ValidRootMotion(snapshot.pendingRootMotion) || snapshot.animation.overlayWeightPermille > 1000U) return false;
