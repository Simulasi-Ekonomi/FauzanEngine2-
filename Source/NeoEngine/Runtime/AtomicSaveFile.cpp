...
bool ReadPath(const std::filesystem::path& path, std::vector<uint8_t>& bytes, AtomicSaveFileError& error) {
    std::error_code ec;
    if (!std::filesystem::exists(path, ec) || ec) { error = AtomicSaveFileError::Missing; return false; }
    const uintmax_t size = std::filesystem::file_size(path, ec);
    if (ec) { error = AtomicSaveFileError::OpenRead; return false; }
    if (size > AtomicSaveFile::kMaxBytes) { error = AtomicSaveFileError::PayloadLimit; return false; }
    std::ifstream stream(path, std::ios::binary);
    if (!stream) { error = AtomicSaveFileError::OpenRead; return false; }
    ...
    if (!stream && size > 0U) { error = AtomicSaveFileError::ReadFailure; return false; }
    bytes = std::move(parsed); error = AtomicSaveFileError::None; return true;
}
...
bool AtomicSaveFile::Write(const std::filesystem::path& root, std::string_view slot, const std::vector<uint8_t>& bytes,
    ...
    if (!ValidSlot(slot)) { error = AtomicSaveFileError::InvalidSlot; return false; }
    if (bytes.size() > kMaxBytes) { error = AtomicSaveFileError::PayloadLimit; return false; }

    std::error_code ec;
    std::filesystem::create_directories(root, ec);
    if (ec) { error = AtomicSaveFileError::CreateDirectory; return false; }
    ...
    if (!WritePath(tempPath, bytes)) {
        std::filesystem::remove(tempPath, ec); error = AtomicSaveFileError::WriteFailure; return false;
    }
    if (!SyncPath(tempPath)) {
        std::filesystem::remove(tempPath, ec); error = AtomicSaveFileError::WriteFailure; return false;
    }
    std::filesystem::rename(tempPath, finalPath, ec);
    if (ec) {
        std::filesystem::remove(tempPath, ec); error = AtomicSaveFileError::RenameFailure; return false;
    }
    if (!Flush(root, slot, error) || !SyncDirectory(root)) {
        if (error == AtomicSaveFileError::None) error = AtomicSaveFileError::WriteFailure;
        return false;
    }
    error = AtomicSaveFileError::None; return true;
}
...
bool AtomicSaveFile::Flush(const std::filesystem::path& root, std::string_view slot, AtomicSaveFileError& error) {
    ...
    if (!SyncPath(path)) { error = AtomicSaveFileError::OpenWrite; return false; }
    error = AtomicSaveFileError::None; return true;
}
...
bool AtomicSaveFile::Backup(const std::filesystem::path& root, std::string_view slot, AtomicSaveFileError& error) {
    ...
    if (!ReadPath(root / (std::string(slot) + ".sav"), bytes, readError)) {
        error = AtomicSaveFileError::BackupFailure; return false;
    }
    ...
    if (!WritePath(tempPath, bytes) || !SyncPath(tempPath)) {
        std::filesystem::remove(tempPath, ec); error = AtomicSaveFileError::BackupFailure; return false;
    }
    std::filesystem::rename(tempPath, backupPath, ec);
    if (ec) {
        std::filesystem::remove(tempPath, ec); error = AtomicSaveFileError::BackupFailure; return false;
    }
    if (!SyncPath(backupPath) || !SyncDirectory(root)) {
        error = AtomicSaveFileError::BackupFailure; return false;
    }
    error = AtomicSaveFileError::None; return true;
}
...
bool AtomicSaveFile::RestoreBackup(const std::filesystem::path& root, std::string_view slot,
    ...
    if (!ReadPath(root / (std::string(slot) + ".bak"), bytes, readError)) {
        error = AtomicSaveFileError::RestoreFailure; return false;
    }
