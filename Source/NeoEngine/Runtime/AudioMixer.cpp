...
bool Normalize3(float v[3]) { const float length=Length3(v); if(!std::isfinite(length)||length<=1.0e-5f)return false; v[0]/=length;v[1]/=length;v[2]/=length;return std::isfinite(v[0])&&std::isfinite(v[1])&&std::isfinite(v[2]); }
bool ValidAttenuationModel(AudioAttenuationModel model) { return model == AudioAttenuationModel::Linear || model == AudioAttenuationModel::InverseSquare || model == AudioAttenuationModel::Logarithmic; }
bool ValidWorldCoordinate(float value) { return std::isfinite(value) && std::fabs(value) <= 1.0e9f; }
}
...
bool AudioMixer::Play(uint32_t id,std::vector<int16_t> samples,uint16_t gainQ8,bool looping,float pitch) {
    if (m_Voices.size()>kMaxVoices) return false;
        if(samples.empty()||samples.size()>kMaxSamplesPerVoice||samples.capacity()>kMaxSamplesPerVoice||gainQ8==0||!std::isfinite(pitch)||pitch<=0.001f||pitch>8.0f)return false;
    ...
    if (samples.capacity() > kMaxSamplesPerVoice) return false;
...
bool AudioMixer::PlaySpatial(const SpatialVoiceParams& params) {
    if (m_Voices.size()>kMaxVoices) return false;
    if(params.id==std::numeric_limits<uint32_t>::max()||params.mono.empty()||params.mono.capacity()<params.mono.size()||params.mono.size()>kMaxSamplesPerVoice||params.mono.capacity()>kMaxSamplesPerVoice||params.gainQ8==0||!std::isfinite(params.pitch)||params.pitch<=0.001f||params.pitch>8.0f||!ValidAttenuationModel(params.attenuation.model))return false;
    for(const auto& voice:m_Voices)if(voice.id==params.id)return false;
    if(m_Voices.size()>=kMaxVoices)return false;
    for(float x:params.position)if(!ValidWorldCoordinate(x))return false;
    ...
    const float distance=std::sqrt(dx*dx+dy*dy+dz*dz); if(!std::isfinite(distance) || distance > 1.0e9f)return false;
    if(!std::isfinite(params.attenuation.minDistance)||!std::isfinite(params.attenuation.maxDistance)||!std::isfinite(params.attenuation.minVolume)||params.attenuation.minDistance>1.0e9f||params.attenuation.maxDistance>1.0e9f)return false;
    if(!std::isfinite(params.attenuation.minDistance)||params.attenuation.minDistance<=0.0f||params.attenuation.maxDistance<params.attenuation.minDistance||params.attenuation.maxDistance<0.0f||params.attenuation.maxDistance<=0.0f||params.attenuation.maxDistance<params.attenuation.minDistance||params.attenuation.minVolume<0.0f||params.attenuation.minVolume>1.0f)return false;
    ...
    if(params.spatialized){if(distance>=maxD)attenuation=minVolume;else if(distance>minD){const float t=(distance-minD)/(maxD-minD);if(!std::isfinite(t))return false;attenuation=params.attenuation.model==AudioAttenuationModel::Linear?1.0f-t:(params.attenuation.model==AudioAttenuationModel::Logarithmic?1.0f-std::log10(1.0f+9.0f*t):1.0f/(1.0f+t*t*(maxD/minD)));attenuation=std::max(minVolume,attenuation);}}
    float pan=0.0f;
    if(params.spatialized&&distance>0.001f){float forward[3]{listenerForward[0],listenerForward[1],listenerForward[2]},up[3]{listenerUp[0],listenerUp[1],listenerUp[2]};if(Normalize3(forward)&&Normalize3(up)){const float fu=Dot3(forward,up);up[0]-=forward[0]*fu;up[1]-=forward[1]*fu;up[2]-=forward[2]*fu;if(Normalize3(up)){const float right[3]{up[1]*forward[2]-up[2]*forward[1],up[2]*forward[0]-up[0]*forward[2],up[0]*forward[1]-up[1]*forward[0]};pan=std::clamp((dx*right[0]+dy*right[1]+dz*right[2])/distance,-1.0f,1.0f);}}}
...
bool AudioMixer::UpdateVoicePosition(uint32_t id,const float position[3]) { if(id==std::numeric_limits<uint32_t>::max()||position==nullptr||m_Voices.size()>kMaxVoices||m_Voices.empty()||!ValidWorldCoordinate(position[0])||!ValidWorldCoordinate(position[1])||!ValidWorldCoordinate(position[2]))return false;for(float value:{position[0],position[1],position[2]})if(!std::isfinite(value))return false;for(auto& voice:m_Voices)if(voice.id==id){voice.position[0]=position[0];voice.position[1]=position[1];voice.position[2]=position[2];return true;}return false; }
bool AudioMixer::UpdateVoicePitch(uint32_t id,float pitch) { if(id==std::numeric_limits<uint32_t>::max()||m_Voices.size()>kMaxVoices||m_Voices.empty()||!std::isfinite(pitch)||pitch<=0.001f||pitch>8.0f)return false;for(auto& voice:m_Voices)if(voice.id==id){voice.pitch=pitch;return true;}return false; }
bool AudioMixer::UpdateVoiceGain(uint32_t id,uint16_t gainQ8) { if(id==std::numeric_limits<uint32_t>::max()||m_Voices.size()>kMaxVoices||m_Voices.empty()||gainQ8==0)return false;for(auto& voice:m_Voices)if(voice.id==id){voice.gain=gainQ8;return true;}return false; }
bool AudioMixer::Stop(uint32_t id) { if(m_Voices.size()>kMaxVoices)return false;if(m_Voices.empty())return false;auto it=std::find_if(m_Voices.begin(),m_Voices.end(),[&](const auto& voice){return voice.id==id;});if(it==m_Voices.end())return false;m_Voices.erase(it);return true; }
void AudioMixer::Clear(){ m_Voices.clear(); m_Voices.shrink_to_fit(); }

bool AudioMixer::SetListener(const AudioListener& listener) {
    for(float value:listener.position)if(!ValidWorldCoordinate(value))return false;
    ...
    if (std::fabs(Dot3(forward, up)) > 0.999f) return false;
    if (!ValidAttenuationModel(AudioAttenuationModel::Linear) || !ValidAttenuationModel(AudioAttenuationModel::InverseSquare) || !ValidAttenuationModel(AudioAttenuationModel::Logarithmic)) return false;
...
void AudioMixer::Mix(size_t frames,std::vector<int16_t>& out) {
    ...
    float listenerForward[3]{m_Listener.forward[0],m_Listener.forward[1],m_Listener.forward[2]};
    float listenerUp[3]{m_Listener.up[0],m_Listener.up[1],m_Listener.up[2]};
    if (!Normalize3(listenerForward) || !Normalize3(listenerUp) || std::fabs(Dot3(listenerForward,listenerUp)) > 0.999f) { out.clear(); return; }
    ...
    m_Voices.erase(std::remove_if(m_Voices.begin(),m_Voices.end(),[](const auto& voice){
        return voice.id==0U || voice.samples.empty() || voice.samples.size()>kMaxSamplesPerVoice || voice.samples.capacity()>kMaxSamplesPerVoice || !std::isfinite(voice.cursorSubframe) ||
               !std::isfinite(voice.pitch) || voice.pitch<=0.001F || voice.pitch>8.0F || !std::isfinite(voice.pan) || voice.pan<-1.0F || voice.pan>1.0F;
    }),m_Voices.end());
    if (m_Voices.size()>kMaxVoices || m_Voices.capacity()>kMaxVoices) { out.clear(); return; }
    for(size_t f=0;f<frames;++f){
        if (frames == 0U || frames > kMaxMixFrames) { out.clear(); return; }
    if (m_Voices.size()>kMaxVoices) { out.clear(); return; }
        ...
        for(auto& voice:m_Voices){
            ...
            const int32_t s0=voice.samples[idx0],s1=voice.samples[idx1];const double interpolatedValue=static_cast<double>(s0)+frac*static_cast<double>(s1-s0);if(!std::isfinite(interpolatedValue)||interpolatedValue>static_cast<double>(std::numeric_limits<int32_t>::max())||interpolatedValue<static_cast<double>(std::numeric_limits<int32_t>::min()))continue;const int32_t interpolated=static_cast<int32_t>(std::llround(interpolatedValue));
            ...
            if(!std::isfinite(voice.cursorSubframe)||voice.cursorSubframe<0.0||voice.cursorSubframe>static_cast<double>(std::numeric_limits<size_t>::max())||!std::isfinite(dynamicGain)||!std::isfinite(dynamicPan)||dynamicGain<0.0f||dynamicGain>256.0f||dynamicPan<-1.0f||dynamicPan>1.0f)continue;
    ...
    if (m_Voices.size()>kMaxVoices || m_Voices.capacity()>kMaxVoices) { out.clear(); return; }
    if (out.size() != frames * 2U || out.capacity() < out.size()) { out.clear(); return; }
