...
bool SceneECSBridge::Rebuild(SceneWorld& scene, ArchetypeManager& ecs) {
    ...
    for (const auto& [key, id] : map_) {
        if (!next.contains(key) && ecs.HasEntity(id)) ecs.DestroyEntity(id);
    }
    map_.swap(next);
...
bool SceneECSBridge::Sync(SceneWorld& scene, ArchetypeManager& ecs, const SceneMeshAdapter& meshes) {
    ...
    for (const SceneEntity entity : scene.AliveEntities()) {
        ...
        const auto meshIt = std::find_if(
            meshes.Instances().begin(), meshes.Instances().end(),
        ...
        if (meshIt != meshes.Instances().end()) {
            if (meshIt->sourceAssetId.empty() || meshIt->sourceHash == 0U) return false;
            const uint64_t materialHash = meshIt->sourceMaterialAssetId.empty() ? 0U : meshIt->sourceMaterialHash;
            if (!meshIt->sourceMaterialAssetId.empty() && materialHash == 0U) return false;
            mask |= COMP_MESH;
            ecs.SetComponentMask(id, mask);
            ecs.SetMeshAssetIdentity(id, meshIt->sourceHash, materialHash);
