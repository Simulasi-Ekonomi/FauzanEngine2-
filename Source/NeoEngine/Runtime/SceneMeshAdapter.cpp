...
bool SphereVisible(RenderCamera& camera, RenderPoint3 center, float radius) {
    RenderPoint3 cameraSpace{}; if(!camera.WorldToCamera(center,cameraSpace)) return false;
    const RenderCameraConfig& config=camera.Config(); const float x=cameraSpace.x, y=cameraSpace.y, z=cameraSpace.z;
    if (!std::isfinite(radius) || radius<0.0F || z+radius<config.nearPlane || z-radius>config.farPlane) return false;
    if (config.mode==RenderCameraMode::Orthographic) { const float halfHeight=config.orthographicHalfHeight, halfWidth=halfHeight*config.aspect; return !(x-radius>halfWidth || x+radius<-halfWidth || y-radius>halfHeight || y+radius<-halfHeight); }
    if (z-radius<=config.nearPlane) return true;
    const float halfHeight=(z+radius)*std::tan(config.verticalFovDegrees*0.00872664626F), halfWidth=halfHeight*config.aspect;
    return !(x-radius>halfWidth || x+radius<-halfWidth || y-radius>halfHeight || y+radius<-halfHeight);
}
}
...
bool SceneMeshAdapter::Draw(const SceneWorld& world,RenderCamera& camera,SoftwareRenderer& renderer,const DirectionalLight& light){lastCulledCount_=0;MeshRenderer meshRenderer;for(const SceneMeshInstance& instance:instances_){const Transform3* transform=world.GetTransform(instance.entity);if(!transform){lastError_=SceneMeshAdapterError::MissingEntity;return false;}if(!Finite(*transform)||transform->sx<=0.0F||std::fabs(transform->sx-transform->sy)>0.0001F||std::fabs(transform->sx-transform->sz)>0.0001F){lastError_=SceneMeshAdapterError::UnsupportedTransform;return false;}const RenderPoint3 translation{transform->x,transform->y,transform->z};if(!SphereVisible(camera,translation,instance.localBoundsRadius*transform->sx)){++lastCulledCount_;continue;}if(!meshRenderer.Draw(instance.vertices,instance.indices,{translation,transform->sx,{transform->rx,transform->ry,transform->rz}},instance.material,light,camera,renderer)){lastError_=SceneMeshAdapterError::DrawFailed;return false;}}lastError_=SceneMeshAdapterError::None;return true;}
} // namespace NeoEngine
