...
bool SphereVisible(const RenderCameraConfig& config, RenderPoint3 center, float radius) {
    const float x=center.x-config.position.x, y=center.y-config.position.y, z=center.z-config.position.z;
...
bool SceneMeshAdapter::Draw(const SceneWorld& world,RenderCamera& camera,SoftwareRenderer& renderer,const DirectionalLight& light){lastCulledCount_=0;MeshRenderer meshRenderer;for(const SceneMeshInstance& instance:instances_){const Transform3* transform=world.GetTransform(instance.entity);if(!transform){lastError_=SceneMeshAdapterError::MissingEntity;return false;}if(!Finite(*transform)||transform->sx<=0.0F||std::fabs(transform->sx-transform->sy)>0.0001F||std::fabs(transform->sx-transform->sz)>0.0001F){lastError_=SceneMeshAdapterError::UnsupportedTransform;return false;}const RenderPoint3 translation{transform->x,transform->y,transform->z};if(!SphereVisible(camera.Config(),translation,instance.localBoundsRadius*transform->sx)){++lastCulledCount_;continue;}if(!meshRenderer.Draw(instance.vertices,instance.indices,{translation,transform->sx,{transform->rx,transform->ry,transform->rz}},instance.material,light,camera,renderer)){lastError_=SceneMeshAdapterError::DrawFailed;return false;}}lastError_=SceneMeshAdapterError::None;return true;}
} // namespace NeoEngine
