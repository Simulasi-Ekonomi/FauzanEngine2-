...
VulkanRenderer3D::~VulkanRenderer3D() { Destroy(); }
...
VulkanRenderer3D::VulkanRenderer3D(VulkanRenderer3D&& other) noexcept {
    ...
    isInitialized_ = other.isInitialized_; inFrame_ = other.inFrame_;
    other.context_ = nullptr; other.currentDescriptorSet_ = VK_NULL_HANDLE; other.commandPool_ = VK_NULL_HANDLE;
    other.width_ = other.height_ = 0; other.isInitialized_ = false; other.inFrame_ = false;
}
...
bool VulkanRenderer3D::Initialize(VulkanContext* context, uint32_t width, uint32_t height) {
    if (context == nullptr || !context->Ready() || width == 0 || height == 0) return false;
    Destroy(); context_ = context; width_ = width; height_ = height;
    VkDevice device = context_->Device(); VkPhysicalDevice physicalDevice = context_->PhysicalDevice();

    RenderPassConfig rpConfig{}; rpConfig.colorFormat = VK_FORMAT_R8G8B8A8_UNORM; rpConfig.enableDepth = false;
    rpConfig.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED; rpConfig.finalLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    if (!renderPassManager_.Initialize(device, rpConfig)) { Destroy(); return false; }
    ...
    if (!descriptorManager_.Initialize(device, bindings, 8)) { Destroy(); return false; }
    if (!cameraBuffer_.Initialize(device, physicalDevice, sizeof(CameraUBO), VulkanBufferType::UniformBuffer, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT) ||
        ...
        !batchInstanceBuffer_.Initialize(device, physicalDevice, sizeof(VulkanMeshInstanceData) * MaxBatchInstances,
                                          VulkanBufferType::VertexBuffer, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)) {
    ...
    VkCommandPoolCreateInfo poolInfo{}; poolInfo.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
    poolInfo.queueFamilyIndex = context_->GraphicsQueueFamily(); poolInfo.flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT;
    if (vkCreateCommandPool(device, &poolInfo, nullptr, &commandPool_) != VK_SUCCESS) { Destroy(); return false; }
    if (!commandRecorder_.Initialize(device, commandPool_) || !indirectRenderer_.Initialize(device, physicalDevice)) { Destroy(); return false; }
    isInitialized_ = true; return true;
}

bool VulkanRenderer3D::BeginFrame() {
    if (!isInitialized_ || inFrame_) return false;
    if (!commandRecorder_.BeginRecording()) return false;
...
void VulkanRenderer3D::DrawMesh(const VulkanMeshBufferBuilder& mesh, const ModelUBO& model, const VulkanGPUTexture* /*texture*/) {
    ...
    commandRecorder_.BindVertexBuffer(mesh.GetVertexBuffer().GetBuffer()); commandRecorder_.BindIndexBuffer(mesh.GetIndexBuffer().GetBuffer());
...
bool VulkanRenderer3D::DrawMeshBatch(const VulkanMeshBatchBuffer& batch) {
    if (!inFrame_ || !batch.IsValid() || batch.MeshCount() > indirectRenderer_.Capacity() || batch.MeshCount() > MaxBatchInstances) return false;
...
bool VulkanRenderer3D::EndFrame() {
    ...
    bool ok = true;
    if (indirectRenderer_.PendingDrawCount() > 0) ok = indirectRenderer_.Execute(commandRecorder_.GetCommandBuffer());
    ok = commandRecorder_.EndRecording() && ok; inFrame_ = false; return ok;
}

void VulkanRenderer3D::Destroy() {
    if (context_ != nullptr && context_->Ready()) {
        VkDevice device = context_->Device(); commandRecorder_.Destroy();
        if (commandPool_ != VK_NULL_HANDLE) { vkDestroyCommandPool(device, commandPool_, nullptr); commandPool_ = VK_NULL_HANDLE; }
        indirectRenderer_.Destroy(); batchInstanceBuffer_.Destroy(); cameraBuffer_.Destroy(); modelBuffer_.Destroy();
        descriptorManager_.Destroy(); graphicsPipeline_.Destroy(); renderPassManager_.Destroy(); context_ = nullptr;
    }
    width_ = height_ = 0; isInitialized_ = false; inFrame_ = false;
}
