...
bool RuntimeVerticalSliceGate::Validate(NeoRuntime& runtime,bool executeTick,VerticalSliceGateReceipt& receipt){
 ...
 if(receipt.sceneEntities==std::numeric_limits<uint64_t>::max()){receipt.error=VerticalSliceGateError::SceneECSMismatch;return false;}
 if(receipt.sceneEntities>std::numeric_limits<uint32_t>::max()){receipt.error=VerticalSliceGateError::SceneECSMismatch;return false;}
 ...
 if(receipt.ecsEntities>std::numeric_limits<uint32_t>::max()){receipt.error=VerticalSliceGateError::SceneECSMismatch;return false;}
 ...
 if(runtime.SceneMeshes()==nullptr){receipt.error=VerticalSliceGateError::SceneMeshMissing;return false;}
 ...
 if(!receipt.assetsValid){receipt.error=VerticalSliceGateError::AssetsMissing;return false;}\n if(!receipt.resourcesValid){receipt.error=VerticalSliceGateError::AssetsMissing;return false;}
 ...
 if(!receipt.replicationValid){receipt.error=VerticalSliceGateError::ReplicationMissing;return false;}
 if(executeTick && (runtime.SceneECS().revision==0U || runtime.SceneECS().revision==std::numeric_limits<uint64_t>::max())){receipt.error=VerticalSliceGateError::SceneECSRevisionMismatch;return false;}
