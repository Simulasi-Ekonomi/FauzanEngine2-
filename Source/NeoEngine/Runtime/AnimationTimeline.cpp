...
namespace NeoEngine {
bool AnimationTimeline::AddTrack(std::string id, std::vector<AnimationKeyframe> keys) { if(id.empty()||id.size()>64U||id.find('\0')!=std::string::npos){lastError_=AnimationError::InvalidTrack;return false;} if(std::any_of(tracks_.begin(),tracks_.end(),[&id](const Track& track){return track.id==id;})){lastError_=AnimationError::DuplicateTrack;return false;} if(tracks_.size()>=kMaxTracks||keys.size()>kMaxKeysPerTrack){lastError_=AnimationError::Capacity;return false;} if(keys.size()<2U||!std::isfinite(keys.front().time)||keys.front().time!=0.0F){lastError_=AnimationError::InvalidKeys;return false;} for(size_t index=0;index<keys.size();++index){if(!std::isfinite(keys[index].time)||!std::isfinite(keys[index].value)||keys[index].time>kMaxDurationSeconds||(index>0U&&keys[index].time<=keys[index-1U].time)){lastError_=AnimationError::InvalidKeys;return false;}}     try { tracks_.push_back({std::move(id),std::move(keys)}); }
...
bool AnimationTimeline::AddEventMarker(std::string trackId, AnimationEventMarker marker) {
    ...
    try { found->events.push_back(std::move(marker)); }
    catch (const std::bad_alloc&) { lastError_ = AnimationError::Capacity; return false; }
...
bool AnimationTimeline::CollectEvents(const std::string& trackId, float fromTime, float toTime, AnimationPlayback playback, std::vector<std::string>& output) const {
    const auto found = std::find_if(tracks_.begin(), tracks_.end(), [&trackId](const Track& track) { return track.id == trackId; });
    if (found == tracks_.end()) { lastError_ = AnimationError::MissingTrack; return false; }
    if (playback != AnimationPlayback::Clamp && playback != AnimationPlayback::Loop) { lastError_ = AnimationError::InvalidPlayback; return false; }
    ...
    const auto appendRange = [&found, &candidate](float start, float end) -> bool {
        try {
    ...
    if (playback == AnimationPlayback::Loop) {
        ...
        if (end >= start) { if (!appendRange(start, end)) { lastError_ = AnimationError::Capacity; return false; } }
        else if (!appendRange(start, duration) || !appendRange(-0.000001F, end)) { lastError_ = AnimationError::Capacity; return false; }
    } else {
        const float start = std::clamp(fromTime, 0.0F, duration);
        const float end = std::clamp(toTime, 0.0F, duration);
        if (end >= start && !appendRange(start, end)) { lastError_ = AnimationError::Capacity; return false; }
    }
...
bool AnimationTimeline::Sample(const std::string& id,float time,AnimationPlayback playback,float& value) const { const auto found=std::find_if(tracks_.begin(),tracks_.end(),[&id](const Track& track){return track.id==id;}); if(found==tracks_.end()){lastError_=AnimationError::MissingTrack;return false;} if(playback != AnimationPlayback::Clamp && playback != AnimationPlayback::Loop){lastError_=AnimationError::InvalidPlayback;return false;} if(!std::isfinite(time)){lastError_=AnimationError::InvalidKeys;return false;} const std::vector<AnimationKeyframe>& keys=found->keys;const float duration=keys.back().time;float sampleTime=time;if(playback==AnimationPlayback::Loop){sampleTime=std::fmod(time,duration);if(sampleTime<0.0F)sampleTime+=duration;}else sampleTime=std::clamp(time,0.0F,duration); if(sampleTime>=duration){value=keys.back().value;lastError_=AnimationError::None;return true;} const auto upper=std::upper_bound(keys.begin(),keys.end(),sampleTime,[](float needle,const AnimationKeyframe& key){return needle<key.time;});const size_t right=static_cast<size_t>(upper-keys.begin()),left=right-1U;const float span=keys[right].time-keys[left].time,alpha=(sampleTime-keys[left].time)/span;value=static_cast<float>(static_cast<double>(keys[left].value)+(static_cast<double>(keys[right].value)-static_cast<double>(keys[left].value))*static_cast<double>(alpha));if(!std::isfinite(value)){lastError_=AnimationError::InvalidKeys;return false;}lastError_=AnimationError::None;return true; }
} // namespace NeoEngine
