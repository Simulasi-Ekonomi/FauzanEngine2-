...
bool FarmPlayerInputBridge::Initialize(FarmPlayerInputBindings bindings) { if (!ValidBindings(bindings)) return Fail(FarmPlayerInputBridgeError::InvalidConfiguration); bindings_ = std::move(bindings); initialized_ = true; lastReceipt_ = {}; lastError_ = FarmPlayerInputBridgeError::None; return true; }
bool FarmPlayerInputBridge::Step(const InputState& input, FarmWorldTool& world) {
    ...
    FarmPlayerInputReceipt candidate{FarmPlayerInputKind::None, selectedAction_, current.x, current.z, 0U};
    if (dx != 0 || dz != 0) {
        ...
        candidate = {FarmPlayerInputKind::Movement, selectedAction_, static_cast<uint16_t>(nextX), static_cast<uint16_t>(nextZ), 0U};
    } else if (interact) {
        bool accepted = false; uint32_t harvested = 0U;
        switch (selectedAction_) {
            ...
            case FarmPlayerAction::Harvest: accepted = world.PlayerHarvest(current.x, current.z, harvested); break;
        }
        if (!accepted) return Fail(FarmPlayerInputBridgeError::WorldRejected);
        candidate = {FarmPlayerInputKind::Action, selectedAction_, current.x, current.z, harvested};
    }
    lastReceipt_ = candidate; lastError_ = FarmPlayerInputBridgeError::None;
    return true;
}
} // namespace NeoEngine
