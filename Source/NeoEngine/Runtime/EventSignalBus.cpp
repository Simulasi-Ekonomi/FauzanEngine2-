...
bool EventSignalBus::Subscribe(RuntimeEventListener& listener) {
    if (std::find(listeners_.begin(), listeners_.end(), &listener) != listeners_.end()) return Fail(EventSignalError::DuplicateListener);
    if (listeners_.size() >= kMaxListeners) return Fail(EventSignalError::Capacity);
    try { listeners_.push_back(&listener); }
    catch (const std::bad_alloc&) { return Fail(EventSignalError::Capacity); }
...
bool EventSignalBus::Queue(RuntimeEvent event) {
    if (pending_.size() >= kMaxEvents) return Fail(EventSignalError::QueueFull);
    try { pending_.push_back(event); }
    catch (const std::bad_alloc&) { return Fail(EventSignalError::Capacity); }
    lastError_ = EventSignalError::None;
    return true;
}
