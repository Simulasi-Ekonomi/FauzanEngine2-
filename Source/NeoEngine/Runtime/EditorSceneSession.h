...
public:
    ...
    bool UpdateActorProperties(uint32_t actorId, const std::string& name, const std::string& materialAssetId, const std::string& textureAssetId, uint32_t spriteRgba, const AssetRegistry& assets);
    ...
    [[nodiscard]] bool HasSelection() const { return selectedActorId_ != 0U || !selectedActorIds_.empty(); }
