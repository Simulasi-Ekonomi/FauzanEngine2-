...
public:
    bool accept(uint64_t sequence) {
        if (!sequence || sequence == std::numeric_limits<uint64_t>::max()) return false;
...
public:
    ...
    bool serverConsume(const NetworkInputCommand& in,NetworkTransformState& out){if(role_!=NetworkRole::Server)return Fail(NetworkError::InvalidInput);if(!validateInput(in))return false;for(auto&e:entities_)if(e.networkId==in.networkId){if(e.ownerId!=in.clientId)return Fail(NetworkError::Ownership);if(!acceptSequence(in.clientId,in.sequence))return false;if(e.revision==std::numeric_limits<uint64_t>::max())return Fail(NetworkError::Capacity);e.x+=in.moveX;e.z+=in.moveZ;++e.revision;out=e;error_=NetworkError::None;return true;}return Fail(NetworkError::InvalidInput);}
    ...
    bool reconcile(const NetworkTransformState& authoritative,uint64_t acknowledged,NetworkReconciliationReceipt& r){if(role_!=NetworkRole::Client||authoritative.ownerId!=peerId_)return Fail(NetworkError::InvalidPeer);if(authoritative.networkId==0U||authoritative.networkId==std::numeric_limits<uint32_t>::max()||authoritative.revision==std::numeric_limits<uint64_t>::max()||!std::isfinite(authoritative.x)||!std::isfinite(authoritative.y)||!std::isfinite(authoritative.z))return Fail(NetworkError::InvalidInput);if(authoritative.revision<revision_)return Fail(NetworkError::Stale);revision_=authoritative.revision;float dx=predictedX_-authoritative.x,dy=predictedY_-authoritative.y,dz=predictedZ_-authoritative.z;r={true,0,acknowledged,revision_,dx*dx+dy*dy+dz*dz};predictedX_=authoritative.x;predictedY_=authoritative.y;predictedZ_=authoritative.z;r.replayedInputs=prediction_.replay(acknowledged,predictedX_,predictedY_,predictedZ_,[](auto const&i,float&x,float&,float&z){x+=i.moveX;z+=i.moveZ;});error_=NetworkError::None;return true;}
    NetworkSnapshot snapshot(uint64_t tick)const{NetworkSnapshot s{}; if(tick==std::numeric_limits<uint64_t>::max()||snapshotSequence_==std::numeric_limits<uint64_t>::max()) return s; s.sequence=++snapshotSequence_;s.serverTick=tick;for(auto const&e:entities_)if(e.networkId)s.states.push_back(e);s.checksum=checksum(s);return s;}
    bool acceptSnapshot(NetworkSnapshot const&s){if(role_!=NetworkRole::Client)return Fail(NetworkError::InvalidInput);if(s.sequence==0U||s.sequence==std::numeric_limits<uint64_t>::max()||s.serverTick==std::numeric_limits<uint64_t>::max()||s.sequence<=lastSnapshot_)return Fail(NetworkError::Duplicate);if(checksum(s)!=s.checksum)return Fail(NetworkError::Checksum);lastSnapshot_=s.sequence;error_=NetworkError::None;return true;}
...
private:
    ...
    bool validateInput(NetworkInputCommand const&i){if(!i.clientId||!i.networkId||!i.sequence||i.sequence==std::numeric_limits<uint64_t>::max()||i.clientTick==std::numeric_limits<uint64_t>::max()||!std::isfinite(i.moveX)||!std::isfinite(i.moveZ))return Fail(NetworkError::InvalidInput);float m=i.moveX*i.moveX+i.moveZ*i.moveZ;if(!std::isfinite(m)||m>1.001f)return Fail(NetworkError::InvalidInput);return true;}
