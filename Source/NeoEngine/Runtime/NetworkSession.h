...
public:
    ...
    bool assignOwnership(uint32_t id,uint32_t owner){if(role_!=NetworkRole::Server||!id||!owner)return Fail(NetworkError::InvalidInput);for(auto&e:entities_)if(e.networkId==id||!e.networkId){if(!e.networkId)e.networkId=id;e.ownerId=owner;error_=NetworkError::None;return true;}return Fail(NetworkError::Capacity);}
    bool serverConsume(const NetworkInputCommand& in,NetworkTransformState& out){if(role_!=NetworkRole::Server)return Fail(NetworkError::InvalidInput);if(!accept(in))return false;for(auto&e:entities_)if(e.networkId==in.networkId){if(e.ownerId!=in.clientId)return Fail(NetworkError::Ownership);e.x+=in.moveX;e.z+=in.moveZ;++e.revision;out=e;error_=NetworkError::None;return true;}return Fail(NetworkError::InvalidInput);}
...
private:
    ...
    bool accept(NetworkInputCommand const&i){if(!i.clientId||!i.networkId||!i.sequence||i.moveX!=i.moveX||i.moveZ!=i.moveZ)return Fail(NetworkError::InvalidInput);float m=i.moveX*i.moveX+i.moveZ*i.moveZ;if(m>1.001f)return Fail(NetworkError::InvalidInput);if(!windows_[i.clientId%128].accept(i.sequence))return Fail(NetworkError::Duplicate);error_=NetworkError::None;return true;}
