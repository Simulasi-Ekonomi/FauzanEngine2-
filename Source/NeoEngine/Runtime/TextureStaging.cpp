...
bool TextureStagingStore::StagePpm(const AssetRegistry& registry, std::string_view assetId) {
    ...
    if (!PpmTextureDecoder::DecodeP6(*bytes, decoded, decodeError)) { lastError_ = TextureStagingError::DecodeFailed; return false; }
    if (decoded.rgba.size() > kMaxDecodedBytes - decodedBytes_) { lastError_ = TextureStagingError::CapacityExceeded; return false; }
    const size_t addedBytes = decoded.rgba.size();
...
bool TextureStagingStore::StageBmp(const AssetRegistry& registry, std::string_view assetId) {
    ...
    if (decoded.rgba.size() > kMaxDecodedBytes - decodedBytes_) { lastError_ = TextureStagingError::CapacityExceeded; return false; }
    const size_t addedBytes = decoded.rgba.size();
    resources_.push_back({std::string(assetId), definition->contentHash, TextureSourceFormat::BmpBiRgb, decoded.width, decoded.height, std::move(decoded.rgba)});
    decodedBytes_ += addedBytes; lastError_ = TextureStagingError::None; return true;
}
...
bool TextureStagingStore::Refresh(const AssetRegistry& registry, std::string_view assetId) {
    ...
    RgbaTexture decoded; TextureDecodeError decodeError = TextureDecodeError::None; bool decodedOk = false;
    if (found->sourceFormat == TextureSourceFormat::PpmP6) decodedOk = PpmTextureDecoder::DecodeP6(*bytes, decoded, decodeError);
    else if (found->sourceFormat == TextureSourceFormat::BmpBiRgb) decodedOk = BmpTextureDecoder::DecodeBiRgb(*bytes, decoded, decodeError);
    ...
    found->sourceHash = definition->contentHash; found->width = decoded.width; found->height = decoded.height; found->rgba = std::move(decoded.rgba);
    decodedBytes_ = retainedBytes + replacementBytes; lastError_ = TextureStagingError::None; return true;
}
...
bool TextureStagingStore::CanRefresh(const AssetRegistry& registry, std::string_view assetId) const {
    ...
    const CpuTextureResource* found = Find(assetId); if (found == nullptr) return false;
    RgbaTexture decoded; TextureDecodeError decodeError = TextureDecodeError::None;
