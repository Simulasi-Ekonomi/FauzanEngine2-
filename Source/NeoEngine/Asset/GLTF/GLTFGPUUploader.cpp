...
bool GLTFGPUUploader::UploadMesh(const std::vector<Vertex>& vertices,
    ...
    for (const Vertex& vertex : vertices) {
        ...
        for (std::size_t i = 0U; i < 4U; ++i) {
            if (vertex.boneIndices[i] >= kMaxSkinningBones || !std::isfinite(vertex.boneWeights[i]) || vertex.boneWeights[i] < 0.0F) return false;
            weightSum += vertex.boneWeights[i];
        }
        if (!std::isfinite(weightSum) || (weightSum > kWeightEpsilon && std::fabs(weightSum - 1.0F) > 1.0e-4F)) return false;
    }
