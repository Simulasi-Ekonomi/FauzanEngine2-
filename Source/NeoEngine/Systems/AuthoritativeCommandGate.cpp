...
AuthorityDecision AuthoritativeCommandGate::Submit(const AuthorityCommand& command, uint64_t serverTick, const CommandHandler& handler) {
    ...
    try {
        if (player->recentCommands.size() < kMaxRecentCommandsPerPlayer) {
            player->recentCommands.reserve(player->recentCommands.size() + 1U);
        }
    } catch (const std::bad_alloc&) {
        return Reject(AuthorityError::Capacity);
    }
