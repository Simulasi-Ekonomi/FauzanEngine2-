...
#include "FraudDetectionSystem.h"
...
private:
    ...
    FraudDetectionSystem* m_FraudDetection = nullptr;
...
public:
    ...
    void SetFraudDetection(FraudDetectionSystem* fd) { m_FraudDetection = fd; }
    ...
    RealMoneyTransaction* BuyItemWithRealMoney(const std::string& buyerId, const std::string& buyerName,
        ...
        // Cek fraud detection (TERINTEGRASI)
        if (m_FraudDetection) {
            if (!m_FraudDetection->ValidateTransaction(buyerId, listing->sellerId, listing->itemSerial, static_cast<int>(listing->price))) {
                LOGE("Transaction blocked by fraud detection");
                listing->status = ListingStatus::LOCKED;
                return nullptr;
            }
        }
