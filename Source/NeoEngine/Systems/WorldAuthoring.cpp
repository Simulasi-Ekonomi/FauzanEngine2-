...
bool WorldAuthoring::Deserialize(std::span<const uint8_t> bytes) { size_t at = 0; uint32_t magic = 0, version = 0; WorldAuthoringConfig config{}; if (!Get(bytes, at, magic) || !Get(bytes, at, version) || !Get(bytes, at, config) || magic != kMagic || version != kVersion) return Fail(WorldAuthoringError::CorruptPersistence); WorldAuthoring restored; if (!restored.Generate(config)) return Fail(WorldAuthoringError::CorruptPersistence); uint32_t biomeCount = 0; if (!Get(bytes, at, biomeCount) || biomeCount != restored.biomes_.size()) return Fail(WorldAuthoringError::CorruptPersistence); for (auto& biome : restored.biomes_) if (!Get(bytes, at, biome) || biome > WorldBiome::Water) return Fail(WorldAuthoringError::CorruptPersistence); uint32_t treeCount = 0; if (!Get(bytes, at, treeCount) || treeCount > kMaxTrees) return Fail(WorldAuthoringError::CorruptPersistence); restored.trees_.clear(); for (uint32_t i = 0; i < treeCount; ++i) { GridCell tree{}; if (!Get(bytes, at, tree) || tree.x >= config.side || tree.z >= config.side) return Fail(WorldAuthoringError::CorruptPersistence); restored.trees_.push_back(tree); } uint32_t buildingCount = 0; if (!Get(bytes, at, buildingCount) || buildingCount > kMaxBuildings) return Fail(WorldAuthoringError::CorruptPersistence); for (uint32_t i = 0; i < buildingCount; ++i) { WorldBuildingPlacement building{}; if (!Get(bytes, at, building) || !restored.PlaceBuilding(building)) return Fail(WorldAuthoringError::CorruptPersistence); } if (at != bytes.size()) return Fail(WorldAuthoringError::CorruptPersistence); *this = std::move(restored); lastError_ = WorldAuthoringError::None; return true; }
