...
#include <cctype>
#include <new>
namespace NeoEngine { namespace {template<class T>void Put(std::vector<uint8_t>&o,T v){for(size_t i=0;i<sizeof(T);++i)o.push_back(static_cast<uint8_t>((static_cast<uint64_t>(v)>>(i*8))&255));}template<class T>bool Get(const std::vector<uint8_t>&b,size_t&n,T&v){if(n+sizeof(T)>b.size())return false;uint64_t x=0;for(size_t i=0;i<sizeof(T);++i)x|=static_cast<uint64_t>(b[n+i])<<(i*8);n+=sizeof(T);v=static_cast<T>(x);return true;}}bool TelemetryOutbox::ValidId(const std::string&id){if(id.empty()||id.size()>96)return false;for(unsigned char c:id)if(!std::isalnum(c)&&c!='-'&&c!='_')return false;return true;}\nbool ValidJsonObject(const std::string& json){if(json.empty()||json.size()>TelemetryOutbox::kMaxEnvelopeBytes||json.front()!='{')return false;Json::CharReaderBuilder builder;Json::Value root;std::string errors;std::istringstream input(json);return Json::parseFromStream(builder,input,&root,&errors)&&root.isObject();}bool TelemetryOutbox::Enqueue(std::string id,std::string json){if(!ValidId(id)||json.empty()||json.size()>kMaxEnvelopeBytes||!ValidJsonObject(json)||std::any_of(m_Pending.begin(),m_Pending.end(),[&](const auto&e){return e.id==id;})||m_Pending.size()==kMaxEnvelopes)return false;try{m_Pending.push_back({std::move(id),std::move(json)});}catch(const std::bad_alloc&){return false;}return true;}bool TelemetryOutbox::Acknowledge(const std::string&id){auto it=std::find_if(m_Pending.begin(),m_Pending.end(),[&](const auto&e){return e.id==id;});if(it==m_Pending.end())return false;m_Pending.erase(it);return true;}bool TelemetryOutbox::AcknowledgeBatch(const std::vector<std::string>& ids) {
    if (ids.empty() || ids.size() > kMaxEnvelopes) return false;
    std::vector<TelemetryEnvelope> candidate = m_Pending;
    for (const std::string& id : ids) {
        auto it = std::find_if(candidate.begin(), candidate.end(), [&](const auto& e) { return e.id == id; });
