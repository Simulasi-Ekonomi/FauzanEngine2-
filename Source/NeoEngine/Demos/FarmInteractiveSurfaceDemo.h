...
struct FarmInteractiveSurfaceDemoReceipt { uint64_t frames = 0U; uint64_t presentedFrames = 0U; uint64_t worldFramebufferHash = 0U; uint64_t hudFramebufferHash = 0U; FarmPlayerAction selectedAction = FarmPlayerAction::Till; uint8_t selectedActionMask = 0U; uint16_t characterX = 0U; uint16_t characterZ = 0U; FarmTelemetrySnapshot telemetry{}; FarmRuntimeInventorySnapshot inventory{}; };
