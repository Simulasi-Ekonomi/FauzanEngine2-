...
bool RunFlipbookSurfaceDemo(const FlipbookSurfaceDemoConfig& config, FlipbookSurfaceDemoReceipt& receipt, FlipbookSurfaceDemoError& error) {
    ...
    FlipbookFrameSelector selector; if (!selector.Initialize({4U,1U,1U,1U,4U})) { error = FlipbookSurfaceDemoError::SelectorFailed; return false; }
    ...
    for (uint32_t index = 0U; index < config.frames; ++index) {
        SpriteSourceRect rect{}; const float sample = static_cast<float>(index) / 4.0F;
        if (!selector.Select(sample, rect) || rect.x != index || rect.y != 0U || rect.width != 1U || rect.height != 1U) { error = FlipbookSurfaceDemoError::SelectorFailed; return false; }
