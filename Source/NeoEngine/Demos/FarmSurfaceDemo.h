...
namespace NeoEngine {
enum class FarmSurfaceDemoError : uint8_t { None, InvalidConfiguration, RuntimeInitializeFailed, FarmSetupFailed, FrameFailed, HudFailed, ArtifactWriteFailed, ShutdownFailed };
...
struct FarmSurfaceDemoReceipt { uint32_t renderedFrames = 0; uint64_t frameHash = 0; uint32_t presentedFrames = 0; uint32_t buildings = 0; uint32_t npcs = 0; uint64_t worldFrameHash = 0; uint64_t hudFrameHash = 0; };
