...
struct MotionAnimationSurfaceDemoReceipt { uint32_t renderedFrames = 0; uint32_t presentedFrames = 0; uint32_t visiblePixels = 0; uint64_t frameHash = 0; float finalX = 0.0F; float sampledAnimation = 0.0F; bool endedLocomoting = false; };

// Finite runtime proof: kinematic motion is the only transform writer, while
// animation receives input as trigger-only state selection and sprite render reads world state.
