...
bool RunMotionAnimationSurfaceDemo(const MotionAnimationSurfaceDemoConfig& config, MotionAnimationSurfaceDemoReceipt& receipt, MotionAnimationSurfaceDemoError& error) {
    ...
    AnimationLocomotionBridge bridge; if (!bridge.Initialize({"start_move","stop_move",0.01F,"idle","move"})) { error = MotionAnimationSurfaceDemoError::BridgeInitializeFailed; return false; } AnimationSpriteTintBinding tintBinding; if (!tintBinding.Initialize({0xFFFFFFFFU,0xFF80D0FFU})) { error = MotionAnimationSurfaceDemoError::TintBindingFailed; return false; }
