...
bool RunFarmSurfaceDemo(const FarmSurfaceDemoConfig& config, FarmSurfaceDemoReceipt& receipt, FarmSurfaceDemoError& error) {
    ...
    const SoftwareRenderer* renderer = runtime.Renderer();
    const SoftwareSurfacePresenter* surface = runtime.SurfacePresenter();
    const FarmWorldSnapshot snapshot = world->Snapshot();
    if (renderer == nullptr || surface == nullptr || surface->PresentedFrameCount() != config.frames || !renderer->WritePpm(config.ppmPath)) { runtime.Shutdown(); error = FarmSurfaceDemoError::ArtifactWriteFailed; return false; }
    receipt = {config.frames, renderer->FrameHash(), static_cast<uint32_t>(surface->PresentedFrameCount()), snapshot.buildings, snapshot.npcs};
