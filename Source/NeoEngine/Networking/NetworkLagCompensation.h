...
public:
    ...
    bool sample(uint64_t targetTick,HistoricalTransform& out)const{
        if(!count_)return false;const HistoricalTransform*best=nullptr;
        for(uint16_t i=0;i<count_;++i){auto const&h=history_[(head_+Capacity-count_+i)%Capacity];if(h.serverTick<=targetTick&&(!best||h.serverTick>best->serverTick))best=&h;}
