...
bool VulkanRHI::CreateSwapchainResources(uint32_t, uint32_t) {
    uint32_t count = 0; if (vkGetSwapchainImagesKHR(m_Device, m_Swapchain, &count, nullptr) != VK_SUCCESS || count == 0) { DestroySwapchainResources(); return false; } m_SwapchainImages.resize(count); if (vkGetSwapchainImagesKHR(m_Device, m_Swapchain, &count, m_SwapchainImages.data()) != VK_SUCCESS) { DestroySwapchainResources(); return false; }
    m_SwapchainViews.resize(count); m_Framebuffers.resize(count); for (uint32_t i = 0; i < count; ++i) { VkImageViewCreateInfo view{VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO}; view.image = m_SwapchainImages[i]; view.viewType = VK_IMAGE_VIEW_TYPE_2D; view.format = m_SwapchainFormat; view.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT; view.subresourceRange.levelCount = 1; view.subresourceRange.layerCount = 1; if (vkCreateImageView(m_Device, &view, nullptr, &m_SwapchainViews[i]) != VK_SUCCESS) { for (auto created : m_SwapchainViews) if (created != VK_NULL_HANDLE) vkDestroyImageView(m_Device, created, nullptr); m_SwapchainViews.clear(); m_SwapchainImages.clear(); return false; } }
    ...
    VkRenderPassCreateInfo pass{VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO}; pass.attachmentCount = 1; pass.pAttachments = &attachment; pass.subpassCount = 1; pass.pSubpasses = &subpass; pass.dependencyCount = 1; pass.pDependencies = &dependency; if (vkCreateRenderPass(m_Device, &pass, nullptr, &m_RenderPass) != VK_SUCCESS) { DestroySwapchainResources(); return false; }
    for (uint32_t i = 0; i < count; ++i) { VkFramebufferCreateInfo fb{VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO}; fb.renderPass = m_RenderPass; fb.attachmentCount = 1; fb.pAttachments = &m_SwapchainViews[i]; fb.width = static_cast<uint32_t>(m_Width); fb.height = static_cast<uint32_t>(m_Height); fb.layers = 1; if (vkCreateFramebuffer(m_Device, &fb, nullptr, &m_Framebuffers[i]) != VK_SUCCESS) { DestroySwapchainResources(); return false; } }
    VkCommandPoolCreateInfo pool{VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO}; pool.queueFamilyIndex = m_QueueFamily; pool.flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT; if (vkCreateCommandPool(m_Device, &pool, nullptr, &m_CommandPool) != VK_SUCCESS) { DestroySwapchainResources(); return false; }
    VkCommandBufferAllocateInfo alloc{VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO}; alloc.commandPool = m_CommandPool; alloc.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY; alloc.commandBufferCount = 1; if (vkAllocateCommandBuffers(m_Device, &alloc, &m_CommandBuffer) != VK_SUCCESS) { DestroySwapchainResources(); return false; }
    VkSemaphoreCreateInfo semaphore{VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO}; if (vkCreateSemaphore(m_Device, &semaphore, nullptr, &m_ImageAvailable) != VK_SUCCESS || vkCreateSemaphore(m_Device, &semaphore, nullptr, &m_RenderFinished) != VK_SUCCESS) { DestroySwapchainResources(); return false; }
    VkFenceCreateInfo fence{VK_STRUCTURE_TYPE_FENCE_CREATE_INFO}; fence.flags = VK_FENCE_CREATE_SIGNALED_BIT; if (vkCreateFence(m_Device, &fence, nullptr, &m_InFlight) != VK_SUCCESS) { DestroySwapchainResources(); return false; } return true;
}
