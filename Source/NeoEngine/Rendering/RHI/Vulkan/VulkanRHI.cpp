...
bool VulkanRHI::Init(void* nativeWindow, int w, int h, const char* appName) {
    ...
    if (vkEnumeratePhysicalDevices(m_Instance, &deviceCount, devices.data()) != VK_SUCCESS) { Shutdown(); return false; }
    for (VkPhysicalDevice candidate : devices) {
        ...
        for (uint32_t family = 0; family < familyCount; ++family) {
            ...
            if ((families[family].queueFlags & VK_QUEUE_GRAPHICS_BIT) == 0 || families[family].queueCount == 0 ||
                vkGetPhysicalDeviceSurfaceSupportKHR(candidate, family, m_Surface, &present) != VK_SUCCESS || present != VK_TRUE) continue;
            uint32_t formatCount = 0, modeCount = 0;
            if (vkGetPhysicalDeviceSurfaceFormatsKHR(candidate, m_Surface, &formatCount, nullptr) != VK_SUCCESS || formatCount == 0 ||
                vkGetPhysicalDeviceSurfacePresentModesKHR(candidate, m_Surface, &modeCount, nullptr) != VK_SUCCESS || modeCount == 0) continue;
            std::vector<VkSurfaceFormatKHR> formats(formatCount); std::vector<VkPresentModeKHR> modes(modeCount);
            vkGetPhysicalDeviceSurfaceFormatsKHR(candidate, m_Surface, &formatCount, formats.data());
            vkGetPhysicalDeviceSurfacePresentModesKHR(candidate, m_Surface, &modeCount, modes.data());
            ...
            vkGetPhysicalDeviceSurfaceCapabilitiesKHR(candidate, m_Surface, &caps);
            const VkExtent2D extent = ChooseExtent(caps, static_cast<uint32_t>(w), static_cast<uint32_t>(h));
            uint32_t imageCount = caps.minImageCount + 1;
            if (caps.maxImageCount != 0 && imageCount > caps.maxImageCount) imageCount = caps.maxImageCount;
            VkSwapchainCreateInfoKHR swapInfo{VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR};
            swapInfo.surface = m_Surface; swapInfo.minImageCount = imageCount; swapInfo.imageFormat = surfaceFormat.format;
            swapInfo.imageColorSpace = surfaceFormat.colorSpace; swapInfo.imageExtent = extent; swapInfo.imageArrayLayers = 1;
            ...
            swapInfo.presentMode = presentMode; swapInfo.clipped = VK_TRUE;
            if (vkCreateSwapchainKHR(m_Device, &swapInfo, nullptr, &m_Swapchain) != VK_SUCCESS || !CreateSwapchainResources(extent.width, extent.height)) { Shutdown(); return false; }
...
bool VulkanRHI::CreateSwapchainResources(uint32_t, uint32_t) {
    uint32_t count = 0;
    if (vkGetSwapchainImagesKHR(m_Device, m_Swapchain, &count, nullptr) != VK_SUCCESS || count == 0) return false;
    m_SwapchainImages.resize(count);
    if (vkGetSwapchainImagesKHR(m_Device, m_Swapchain, &count, m_SwapchainImages.data()) != VK_SUCCESS) return false;
    m_SwapchainViews.resize(count);
    m_Framebuffers.resize(count);
    for (uint32_t i = 0; i < count; ++i) {
        VkImageViewCreateInfo view{VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO};
        view.image = m_SwapchainImages[i]; view.viewType = VK_IMAGE_VIEW_TYPE_2D; view.format = m_SwapchainFormat == VK_FORMAT_UNDEFINED ? VK_FORMAT_B8G8R8A8_SRGB : m_SwapchainFormat;
        ...
        if (vkCreateImageView(m_Device, &view, nullptr, &m_SwapchainViews[i]) != VK_SUCCESS) return false;
    }
    VkAttachmentDescription attachment{};
    attachment.format = m_SwapchainFormat == VK_FORMAT_UNDEFINED ? VK_FORMAT_B8G8R8A8_SRGB : m_SwapchainFormat;
    ...
    VkRenderPassCreateInfo pass{VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO}; pass.attachmentCount = 1; pass.pAttachments = &attachment; pass.subpassCount = 1; pass.pSubpasses = &subpass; pass.dependencyCount = 1; pass.pDependencies = &dependency;
    if (vkCreateRenderPass(m_Device, &pass, nullptr, &m_RenderPass) != VK_SUCCESS) return false;
    for (uint32_t i = 0; i < count; ++i) {
        VkFramebufferCreateInfo fb{VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO}; fb.renderPass = m_RenderPass; fb.attachmentCount = 1; fb.pAttachments = &m_SwapchainViews[i]; fb.width = static_cast<uint32_t>(m_Width > 0 ? m_Width : 1); fb.height = static_cast<uint32_t>(m_Height > 0 ? m_Height : 1); fb.layers = 1;
        if (vkCreateFramebuffer(m_Device, &fb, nullptr, &m_Framebuffers[i]) != VK_SUCCESS) return false;
    }
    VkCommandPoolCreateInfo pool{VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO}; pool.queueFamilyIndex = m_QueueFamily; pool.flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT;
    if (vkCreateCommandPool(m_Device, &pool, nullptr, &m_CommandPool) != VK_SUCCESS) return false;
    VkCommandBufferAllocateInfo alloc{VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO}; alloc.commandPool = m_CommandPool; alloc.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY; alloc.commandBufferCount = 1;
    if (vkAllocateCommandBuffers(m_Device, &alloc, &m_CommandBuffer) != VK_SUCCESS) return false;
    VkSemaphoreCreateInfo semaphore{VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO};
    if (vkCreateSemaphore(m_Device, &semaphore, nullptr, &m_ImageAvailable) != VK_SUCCESS || vkCreateSemaphore(m_Device, &semaphore, nullptr, &m_RenderFinished) != VK_SUCCESS) return false;
    VkFenceCreateInfo fence{VK_STRUCTURE_TYPE_FENCE_CREATE_INFO}; fence.flags = VK_FENCE_CREATE_SIGNALED_BIT;
    return vkCreateFence(m_Device, &fence, nullptr, &m_InFlight) == VK_SUCCESS;
}
...
void VulkanRHI::DestroySwapchainResources() {
    ...
    for (auto fb : m_Framebuffers) if (fb != VK_NULL_HANDLE) vkDestroyFramebuffer(m_Device, fb, nullptr);
    if (m_RenderPass != VK_NULL_HANDLE) vkDestroyRenderPass(m_Device, m_RenderPass, nullptr);
    for (auto view : m_SwapchainViews) if (view != VK_NULL_HANDLE) vkDestroyImageView(m_Device, view, nullptr);
    if (m_Swapchain != VK_NULL_HANDLE) vkDestroySwapchainKHR(m_Device, m_Swapchain, nullptr);
    m_Framebuffers.clear(); m_SwapchainViews.clear(); m_SwapchainImages.clear();
    m_InFlight = VK_NULL_HANDLE; m_RenderFinished = VK_NULL_HANDLE; m_ImageAvailable = VK_NULL_HANDLE; m_CommandPool = VK_NULL_HANDLE; m_CommandBuffer = VK_NULL_HANDLE; m_RenderPass = VK_NULL_HANDLE; m_Swapchain = VK_NULL_HANDLE;
}
...
void VulkanRHI::Shutdown() {
    ...
    m_Instance = VK_NULL_HANDLE; m_GPU = VK_NULL_HANDLE; m_Device = VK_NULL_HANDLE; m_GraphicsQueue = VK_NULL_HANDLE; m_Surface = VK_NULL_HANDLE;
    m_SwapchainFormat = VK_FORMAT_UNDEFINED; m_QueueFamily = UINT32_MAX; m_ImageIndex = UINT32_MAX; m_Width = 0; m_Height = 0; m_FrameActive = false; m_ImageAcquired = false; m_Initialized = false;
}
...
void VulkanRHI::BeginFrame() {
    ...
    VkClearValue clear{}; clear.color = {{0.02F, 0.02F, 0.025F, 1.0F}};
    VkRenderPassBeginInfo pass{VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO}; pass.renderPass = m_RenderPass; pass.framebuffer = m_Framebuffers[m_ImageIndex]; pass.renderArea.extent = {static_cast<uint32_t>(m_Width), static_cast<uint32_t>(m_Height)}; pass.clearValueCount = 1; pass.pClearValues = &clear;
    vkCmdBeginRenderPass(m_CommandBuffer, &pass, VK_SUBPASS_CONTENTS_INLINE);
    m_ImageAcquired = true; m_FrameActive = true;
}

void VulkanRHI::EndFrame() {
    if (!m_FrameActive || !m_ImageAcquired) return;
    vkCmdEndRenderPass(m_CommandBuffer);
    if (vkEndCommandBuffer(m_CommandBuffer) != VK_SUCCESS) { m_FrameActive = false; m_ImageAcquired = false; return; }
    VkPipelineStageFlags waitStage = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
    VkSubmitInfo submit{VK_STRUCTURE_TYPE_SUBMIT_INFO}; submit.waitSemaphoreCount = 1; submit.pWaitSemaphores = &m_ImageAvailable; submit.pWaitDstStageMask = &waitStage; submit.commandBufferCount = 1; submit.pCommandBuffers = &m_CommandBuffer; submit.signalSemaphoreCount = 1; submit.pSignalSemaphores = &m_RenderFinished;
    if (vkQueueSubmit(m_GraphicsQueue, 1, &submit, m_InFlight) != VK_SUCCESS) { m_FrameActive = false; m_ImageAcquired = false; return; }
    m_FrameActive = false;
}

void VulkanRHI::Present() {
    if (m_Swapchain == VK_NULL_HANDLE || m_ImageIndex == UINT32_MAX) return;
    VkPresentInfoKHR present{VK_STRUCTURE_TYPE_PRESENT_INFO_KHR}; present.waitSemaphoreCount = 1; present.pWaitSemaphores = &m_RenderFinished; present.swapchainCount = 1; present.pSwapchains = &m_Swapchain; present.pImageIndices = &m_ImageIndex;
    vkQueuePresentKHR(m_GraphicsQueue, &present);
    m_ImageAcquired = false; m_ImageIndex = UINT32_MAX;
}
} // namespace NeoEngine
