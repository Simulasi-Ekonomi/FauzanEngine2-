...
void VulkanRHI::BeginFrame() { if (!m_Initialized || m_FrameActive || m_Swapchain == VK_NULL_HANDLE || m_InFlight == VK_NULL_HANDLE) return; if (vkWaitForFences(m_Device, 1, &m_InFlight, VK_TRUE, UINT64_MAX) != VK_SUCCESS) return; const VkResult acquired = vkAcquireNextImageKHR(m_Device, m_Swapchain, UINT64_MAX, m_ImageAvailable, VK_NULL_HANDLE, &m_ImageIndex); if (acquired != VK_SUCCESS && acquired != VK_SUBOPTIMAL_KHR) { m_ImageIndex = UINT32_MAX; return; } if (m_ImageIndex >= m_Framebuffers.size()) { m_ImageIndex = UINT32_MAX; return; } if (vkResetCommandBuffer(m_CommandBuffer, 0) != VK_SUCCESS) { m_ImageIndex = UINT32_MAX; return; } VkCommandBufferBeginInfo begin{VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO}; if (vkBeginCommandBuffer(m_CommandBuffer, &begin) != VK_SUCCESS) { m_ImageIndex = UINT32_MAX; return; } if (vkResetFences(m_Device, 1, &m_InFlight) != VK_SUCCESS) { (void)vkEndCommandBuffer(m_CommandBuffer); m_ImageIndex = UINT32_MAX; return; } VkClearValue clear{}; clear.color = {{0.02F, 0.02F, 0.025F, 1.0F}}; VkRenderPassBeginInfo pass{VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO}; pass.renderPass = m_RenderPass; pass.framebuffer = m_Framebuffers[m_ImageIndex]; pass.renderArea.extent = {static_cast<uint32_t>(m_Width), static_cast<uint32_t>(m_Height)}; pass.clearValueCount = 1; pass.pClearValues = &clear; vkCmdBeginRenderPass(m_CommandBuffer, &pass, VK_SUBPASS_CONTENTS_INLINE); m_ImageAcquired = true; m_FrameActive = true; m_FrameSubmitted = false; }
