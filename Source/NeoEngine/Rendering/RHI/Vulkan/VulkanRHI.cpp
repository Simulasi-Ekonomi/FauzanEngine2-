...
uint32_t FindMemoryType(VkPhysicalDevice physical, uint32_t bits, VkMemoryPropertyFlags flags) {
    ...
    for (uint32_t i = 0; i < properties.memoryTypeCount; ++i) if ((bits & (1U << i)) != 0 && (properties.memoryTypes[i].propertyFlags & flags) == flags) return i;
    return UINT32_MAX;
}
...
void VulkanRHI::EndFrame() { if (!m_FrameActive || !m_ImageAcquired) return; vkCmdEndRenderPass(m_CommandBuffer); if (vkEndCommandBuffer(m_CommandBuffer) != VK_SUCCESS) { m_FrameActive = false; m_ImageAcquired = false; return; } VkPipelineStageFlags waitStage = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT; VkSubmitInfo submit{VK_STRUCTURE_TYPE_SUBMIT_INFO}; submit.waitSemaphoreCount = 1; submit.pWaitSemaphores = &m_ImageAvailable; submit.pWaitDstStageMask = &waitStage; submit.commandBufferCount = 1; submit.pCommandBuffers = &m_CommandBuffer; submit.signalSemaphoreCount = 1; submit.pSignalSemaphores = &m_RenderFinished; if (vkQueueSubmit(m_GraphicsQueue, 1, &submit, m_InFlight) != VK_SUCCESS) { m_FrameActive = false; m_ImageAcquired = false; return; } m_FrameActive = false; }
void VulkanRHI::Present() { if (m_Swapchain == VK_NULL_HANDLE || m_ImageIndex == UINT32_MAX || m_ImageAcquired) return; VkPresentInfoKHR present{VK_STRUCTURE_TYPE_PRESENT_INFO_KHR}; present.waitSemaphoreCount = 1; present.pWaitSemaphores = &m_RenderFinished; present.swapchainCount = 1; present.pSwapchains = &m_Swapchain; present.pImageIndices = &m_ImageIndex; vkQueuePresentKHR(m_GraphicsQueue, &present); m_ImageAcquired = false; m_ImageIndex = UINT32_MAX; }
} // namespace NeoEngine
