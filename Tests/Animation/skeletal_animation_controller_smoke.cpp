...
int main() {
    ...
    if (skeleton.GetBoneCount() != 3U || !skeleton.IsComplete()) return 1;
    if (skeleton.GetBone(0U) == nullptr || skeleton.GetBone(1U) == nullptr || skeleton.GetBone(2U) == nullptr) return 1;
    SkeletalPoseClip clip; if (!clip.Configure(3U) || clip.BoneCount() != 3U || !clip.SetTrack(0U, std::vector<SkeletalPoseKeyframe>{Key(0.0F, 0.0F, 0.0F, 0.0F), Key(1.0F, 2.0F, 0.0F, 0.0F)}) || !clip.SetTrack(1U, std::vector<SkeletalPoseKeyframe>{Key(0.0F, 0.0F, 2.0F, 0.0F)}) || !clip.SetTrack(2U, std::vector<SkeletalPoseKeyframe>{Key(0.0F, 0.0F, 0.0F, 3.0F)})) return 1;
    ...
    if (controller.LastError() != SkeletalAnimationControllerError::None) return 1;
    ...
    if (controller.Time() != 0.5F) return 1;
    ...
    if (palette.capacity() < palette.size()) return 1;
    ...
    if (positions.size() == 0U || normals.size() == 0U) return 1;
    ...
    if (positions.size() != 3U || normals.size() != 3U) return 1;
    ...
    if (world.GetLocalTransform(actor) == nullptr) return 1;
