...
int main() {
    ...
    ok = ok && world.Initialize(farm, trust, "player-a", worldConfig) && ledger.Initialize(world, "player-a", [](const FarmProviderReceipt& receipt) { return receipt.authorityPayload == "provider-ok"; });
    ...
    ok = ok && !ledger.Apply({1U, "player-a", 25, "provider-ok", false}, audit) && ledger.LastError() == FarmCommerceError::Duplicate && farm.Coins() == initialCoins + 25;
    ...
    ok = ok && !ledger.Reconcile(1U, 24, audit) && ledger.LastError() == FarmCommerceError::ReconciliationMismatch && ledger.Reconcile(1U, 25, audit) && audit.kind == FarmCommerceAuditKind::Approved && audit.providerReceiptId == 1U;
    if (!ok) { std::fprintf(stderr, "FARM_COMMERCE_ENTITLEMENT_SMOKE_FAIL\n"); return 1; }
    std::printf("FARM_COMMERCE_ENTITLEMENT_SMOKE_OK approved=1 duplicate=1 reversal=1 reconcile=1\n");
    return 0;
}
