...
int main() {
    ...
    if(!timeline.AddTrack("extreme",{{0.0F,-extreme},{1.0F,extreme}})||!timeline.Sample("extreme",0.5F,AnimationPlayback::Clamp,value)||!std::isfinite(value)||std::abs(value)>1.0F) return 1;
    if(timeline.AddTrack("too-long",{{0.0F,0.0F},{AnimationTimeline::kMaxDurationSeconds+0.1F,1.0F}})||timeline.LastError()!=AnimationError::InvalidKeys) return 1;
    ...
    std::printf("ANIMATION_TIMELINE_SMOKE_OK tracks=2 interpolate=1 clamp=1 loop=1 markers=1 validation=1\n"); return 0;
}
