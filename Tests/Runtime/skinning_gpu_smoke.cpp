...
int main() {
    ...
    if (skinning.UploadBones({})) return 2;
    ...
    palette[1][12] = 1.0F;
    if (!skinning.UploadBones(palette) || skinning.UploadedBoneCount() != 2U || !skinning.IsValid()) return 4;
    ...
    skinning.Destroy();
    if (skinning.IsValid() || skinning.GetBoneBuffer() != VK_NULL_HANDLE || skinning.UploadedBoneCount() != 0U) return 6;
    context.Reset();
