...
int main() {
    ...
    playerConfig.farmWidth = 4U; playerConfig.farmHeight = 4U; playerConfig.enableFarmPlayerInput = true;
    ...
    ok = ok && playerRuntime.Input()->Push(MakeInputCode(InputDeviceType::Keyboard, 23U), false) && playerRuntime.Tick() && playerRuntime.FarmPlayerInput()->SelectedAction() == FarmPlayerAction::Till && playerRuntime.Input()->Push(MakeInputCode(InputDeviceType::Keyboard, 24U), true) && playerRuntime.Tick() && playerRuntime.Farm()->TileStateAt(1U, 0U) == FarmTileState::Tilled && playerRuntime.LastFrameReceipt()->hasFarmPlayerInputReceipt && playerRuntime.LastFrameReceipt()->farmPlayerInput.kind == FarmPlayerInputKind::Action && playerRuntime.LastFrameReceipt()->farmPlayerInput.action == FarmPlayerAction::Till && playerRuntime.LastFrameReceipt()->farmPlayerInput.x == 1U && playerRuntime.LastFrameReceipt()->farmPlayerInput.z == 0U && playerRuntime.Shutdown();
    if (!ok) {
