...
int main() {
    ...
    ok = ok && firstTick && firstReceipt && firstReceipt->clock.frameCount == 1U && firstReceipt->clock.fixedStepCount == 0U && firstReceipt->time.hostFixedStepCount == 2U && firstReceipt->farm.simulationTick == 2U && firstReceipt->world.simulationTick == 2U && firstReceipt->input.boundActions == 0U && firstReceipt->input.pendingEvents == 0U && firstReceipt->assets.assetCount == 1U && firstReceipt->assets.readyAssetCount == 1U && firstReceipt->assets.storedByteCount == 0U && firstReceipt->sceneAliveEntityCount == scene->AliveCount() && firstReceipt->sceneAliveEntityCount > 1U && !firstReceipt->hasFarmRenderReceipt && runtime.RenderFarm() && runtime.LastFrameReceipt()->hasFarmRenderReceipt && runtime.LastFrameReceipt()->farmRender.frame == 1U && runtime.LastFrameReceipt()->farmRender.worldFramebufferHash == runtime.Renderer()->FrameHash();
