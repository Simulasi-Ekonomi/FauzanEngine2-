...
int main() {
    ...
    RuntimeConfig playerConfig{};
    playerConfig.farmWidth = 4U; playerConfig.farmHeight = 4U; playerConfig.enableFarmPlayerInput = true; playerConfig.enableFarmRuntimeHud = true;
    NeoRuntime playerRuntime;
    ok = ok && playerRuntime.Initialize(playerConfig) && playerRuntime.Input() && playerRuntime.FarmPlayerInput() && playerRuntime.Input()->Push(MakeInputCode(InputDeviceType::Keyboard, 23U), true) && playerRuntime.Tick() && playerRuntime.FarmWorld()->Character().x == 1U && playerRuntime.LastFrameReceipt()->hasFarmPlayerInputReceipt && playerRuntime.LastFrameReceipt()->farmPlayerInput.kind == FarmPlayerInputKind::Movement && playerRuntime.LastFrameReceipt()->farmPlayerInput.x == 1U;
    FarmActionPanelReceipt hudReceipt{};
    ok = ok && !runtime.RouteFarmHudKeyboard(UiKeyboardKey::TabForward, hudReceipt) && runtime.LastError() == RuntimeError::InvalidState && playerRuntime.Input()->Push(MakeInputCode(InputDeviceType::Keyboard, 23U), false) && playerRuntime.Tick() && playerRuntime.RenderFarm() && playerRuntime.RouteFarmHudKeyboard(UiKeyboardKey::TabForward, hudReceipt) && playerRuntime.RouteFarmHudKeyboard(UiKeyboardKey::Activate, hudReceipt) && playerRuntime.Farm()->TileStateAt(1U, 0U) == FarmTileState::Empty && hudReceipt.selected && hudReceipt.selectedAction == FarmPlayerAction::Till && playerRuntime.FarmPlayerInput()->SelectedAction() == FarmPlayerAction::Till && playerRuntime.Input()->Push(MakeInputCode(InputDeviceType::Keyboard, 24U), true) && playerRuntime.Tick() && playerRuntime.Farm()->TileStateAt(1U, 0U) == FarmTileState::Tilled && playerRuntime.LastFrameReceipt()->hasFarmPlayerInputReceipt && playerRuntime.LastFrameReceipt()->farmPlayerInput.kind == FarmPlayerInputKind::Action && playerRuntime.LastFrameReceipt()->farmPlayerInput.action == FarmPlayerAction::Till && playerRuntime.LastFrameReceipt()->farmPlayerInput.x == 1U && playerRuntime.LastFrameReceipt()->farmPlayerInput.z == 0U && playerRuntime.RenderFarm() && playerRuntime.LastFrameReceipt()->hasFarmRenderReceipt && playerRuntime.LastFrameReceipt()->farmRender.hudFramebufferHash != 0U && playerRuntime.Shutdown();
    if (!ok) {
