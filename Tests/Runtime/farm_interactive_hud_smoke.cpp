...
int main(){
    ...
    if(!selectAction(22.0F,FarmPlayerAction::PlantWheat,12U)||!input.Push(kInteract,true)||!session.Frame(input)||session.LastFrameReceipt().telemetry.growingTiles!=1U||session.LastFrameReceipt().inventory.wheatSeeds!=31U)return 1;
    ...
    if(!session.DrawHud(hud,overlay)||overlay.hudFramebufferHash==harvestedWorldHash||overlay.inventory.wheatSeeds!=31U||overlay.inventory.wheatProduce!=2U||overlay.telemetry.growingTiles!=0U||overlay.telemetry.harvestableTiles!=0U||overlay.telemetry.questHarvestProgress!=1U||session.FrameCount()!=17U)return 1;
    std::printf("FARM_INTERACTIVE_HUD_SMOKE_OK frame=%llu hud=1 actions=till,plant,water,harvest canonicalInteract=1 wheatSeeds=%u wheatProduce=%u\n",static_cast<unsigned long long>(overlay.frame),overlay.inventory.wheatSeeds,overlay.inventory.wheatProduce);return 0;
}
