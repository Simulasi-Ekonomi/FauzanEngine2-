...
int main(){
    ...
    const uint64_t iconHudHash=renderer.FrameHash();FarmRuntimeHudReceipt staleReceipt{};staleReceipt.frame=99U;staleReceipt.worldFramebufferHash=88U;staleReceipt.hudFramebufferHash=77U;staleReceipt.telemetry.simulationTick=66U;staleReceipt.telemetry.stateRevision=55U;staleReceipt.telemetry.eventSequence=44U;staleReceipt.telemetry.coins=33;staleReceipt.telemetry.tilledTiles=2U;staleReceipt.telemetry.growingTiles=1U;staleReceipt.telemetry.lastError=FarmError::None;staleReceipt.inventory={12U,34U};if(!assets.ReplaceBytes("tile.harvestable",Ppm(201U,202U,203U))||session.DrawHud(hud,staleReceipt)||session.LastError()!=FarmRuntimeSessionError::HudRejected||staleReceipt.frame!=99U||staleReceipt.hudFramebufferHash!=77U||renderer.FrameHash()!=iconHudHash||!session.Frame(input))return 1;
