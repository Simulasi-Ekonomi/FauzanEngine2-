...
int main() {
    ...
    std::vector<std::vector<EntityID>> overlapSets{{999U}}; const std::vector<GameplayOverlapCircle2> overlapBatch{{2,0,0.25F,COLLISION_LAYER_DYNAMIC},{20,0,0.25F,COLLISION_LAYER_DYNAMIC}};
    if(!query.OverlapCircleSet(physics,overlapBatch,overlapSets)||query.LastError()!=GameplayPhysicsQueryError::None||overlapSets.size()!=2U||overlapSets[0].size()!=2U||!overlapSets[1].empty()||!std::is_sorted(overlapSets[0].begin(),overlapSets[0].end()))return fail("batch"); const std::vector<std::vector<EntityID>> preservedSets=overlapSets;
    if(query.OverlapCircleSet(physics,{{2,0,0.25F,COLLISION_LAYER_DYNAMIC},{0,0,-1,COLLISION_LAYER_DYNAMIC}},overlapSets)||query.LastError()!=GameplayPhysicsQueryError::InvalidShape||overlapSets!=preservedSets||query.OverlapCircleSet(physics,{},overlapSets)||query.LastError()!=GameplayPhysicsQueryError::InvalidBatch||overlapSets!=preservedSets)return fail("batchInvalid"); std::vector<GameplayOverlapCircle2> oversized(GameplayPhysicsQuery::kMaxOverlapBatch+1U,{2,0,0.25F,COLLISION_LAYER_DYNAMIC}); if(query.OverlapCircleSet(physics,oversized,overlapSets)||query.LastError()!=GameplayPhysicsQueryError::BatchCapacity||overlapSets!=preservedSets)return fail("batchCapacity");
    JobSystem::Get().Shutdown(); std::printf("GAMEPLAY_PHYSICS_QUERY_SMOKE_OK static=1 dynamic=1 sorted=1 batch=2 atomic=1 noStepWrite=1\n"); return 0;
}
