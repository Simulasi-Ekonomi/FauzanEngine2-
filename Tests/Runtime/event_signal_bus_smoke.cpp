...
int main() {
    ...
    assert(bus.PendingCount() == 1U);
    EventSignalDispatchReceipt receipt{};
    assert(bus.Dispatch(&receipt));
    assert(receipt.eventCount == 1U);
    assert(receipt.listenerCount == 1U);
    assert(bus.LastError() == EventSignalError::None);
