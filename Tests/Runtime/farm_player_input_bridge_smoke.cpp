...
int main() {
    ...
    if (!require(Frame(input, kRight, true) && bridge.Step(input, world) && world.Character().x == 2U && world.Character().z == 1U, "move-right")) return 1;
    if (!require(Frame(input, kRight, false) && bridge.Step(input, world) && Frame(input, kInteract, true) && bridge.Step(input, world) && farm.TileStateAt(2, 1) == FarmTileState::Tilled, "till")) return 1;
    ...
    const FarmCharacterState preserved = world.Character();
    if (!require(Frame(input, kLeft, true) && input.Push(kInteract, true) && (input.BeginFrame(), !bridge.Step(input, world)) && bridge.LastError() == FarmPlayerInputBridgeError::ConflictingInput && world.Character().x == preserved.x && world.Character().z == preserved.z, "conflict")) return 1;
    ...
    std::printf("FARM_PLAYER_INPUT_BRIDGE_SMOKE_OK movement=1 farmActions=3 conflict=1 bounds=1 noEconomyAuthority=1\n");
    return 0;
}
