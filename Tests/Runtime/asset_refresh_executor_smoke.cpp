...
int main() {
    ...
    if(!registry.ReplaceBytes("tile",greenPpm)||!diagnostics.BuildPlan(registry,"tile",textures,meshes,materials,scene)||diagnostics.Entries().size()!=2U||!executor.ExecuteAtomic(diagnostics.Entries(),registry,textures,meshes,materials,scene)||executor.LastError()!=AssetRefreshExecutorError::None||executor.Receipts().size()!=2U||!textures.IsCurrent(registry,"tile"))return 1;
    texture=textures.Find("tile");uint32_t greenPixel=0U;if(texture==nullptr||!pixel(greenPixel)||greenPixel!=0xFF00FF00U)return 1;const uint64_t greenHash=texture->sourceHash;const std::vector<AssetRefreshReceipt> atomicReceipts=executor.Receipts();const std::vector<AssetRefreshPreflightReceipt> atomicPreflight=executor.PreflightReceipts();
    if(!registry.ReplaceBytes("tile",yellowPpm)||!diagnostics.BuildPlan(registry,"tile",textures,meshes,materials,scene)||diagnostics.Entries().size()!=2U||!registry.ReplaceBytes("tile",badPpm)||executor.ExecuteAtomic(diagnostics.Entries(),registry,textures,meshes,materials,scene)||executor.LastError()!=AssetRefreshExecutorError::PlanStale||executor.Receipts().size()!=atomicReceipts.size()||executor.Receipts()[0].assetId!=atomicReceipts[0].assetId||executor.PreflightReceipts().size()!=atomicPreflight.size()||texture->sourceHash!=greenHash||textures.IsCurrent(registry,"tile"))return 1;
