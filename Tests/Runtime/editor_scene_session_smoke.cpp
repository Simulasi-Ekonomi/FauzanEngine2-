...
int main() {
    ...
    EditorSceneDocument invalid=document; invalid.revision=2; invalid.actors[0].assetId="missing.sprite"; if(session.Open(invalid,assets)||session.LastError()!=EditorSceneSessionError::DocumentLoadFailed||!renderer.Clear(0xFF000000U)||!session.RenderViewport(camera,renderer,{{0,0,-1}})||renderer.FrameHash()!=hash)return 1;
    std::printf("EDITOR_SCENE_SESSION_SMOKE_OK hierarchy=1 inspector=1 save=1 viewport=1 atomicOpen=1 hash=%llu\n",static_cast<unsigned long long>(hash)); return 0;
}
