...
int main() {
    ...
    const auto hierarchy=session.HierarchySnapshot(); EditorSceneActor inspected{}; EditorSceneDocument saved{}; if(hierarchy.size()!=2U||hierarchy[0].id!=5U||hierarchy[1].id!=10U||!session.InspectActor(10,inspected)||inspected.assetId!="session.sprite"||!session.SelectActor(10)||!session.HasSelection()||session.SelectedActorId()!=10U||session.SelectActor(99)||session.LastError()!=EditorSceneSessionError::UnknownActor||session.SelectedActorId()!=10U||!session.Save(saved)||saved.sceneId!=document.sceneId)return 1;
    ...
    std::printf("EDITOR_SCENE_SESSION_SMOKE_OK hierarchy=1 inspector=1 selection=1 save=1 viewport=1 edit=1 reparent=1 addDelete=1 atomicOpen=1 hash=%llu\n",static_cast<unsigned long long>(movedHash)); return 0;
}
