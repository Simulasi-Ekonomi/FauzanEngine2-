...
int main() {
    ...
    if(binding.Configure({"idle","idle"}) || binding.LastError()!=AnimationPlaybackControlBindingError::InvalidConfiguration || binding.Configure({std::string("idle\0bad",8U),"move"}) || binding.LastError()!=AnimationPlaybackControlBindingError::InvalidConfiguration || binding.Configure({"idle",std::string("move\0bad",8U)}) || binding.LastError()!=AnimationPlaybackControlBindingError::InvalidConfiguration) return 1;
