...
int main() {
    ...
    if(!machine.Start("idle")||!machine.Snapshot(snapshot)||snapshot.activeStateId!="idle"||snapshot.blending||snapshot.activeTimeSeconds!=0.0F||!machine.Update(0.25F)||!machine.Sample(timeline,value)||std::fabs(value)>0.0001F||!machine.Trigger("idle_to_walk")||!machine.IsBlending()||!machine.Snapshot(snapshot)||snapshot.targetStateId!="walk"||!snapshot.blending||std::fabs(snapshot.activeTimeSeconds-0.25F)>0.0001F||snapshot.targetTimeSeconds!=0.0F||snapshot.blendFraction!=0.0F||!machine.Sample(timeline,value)||std::fabs(value)>0.0001F)return 1;
    if(!machine.Update(0.25F)||!machine.Snapshot(snapshot)||std::fabs(snapshot.blendFraction-0.5F)>0.0001F||std::fabs(snapshot.targetTimeSeconds-0.25F)>0.0001F||!machine.Sample(timeline,value)||std::fabs(value-6.25F)>0.0001F||machine.Trigger("idle_to_walk")||machine.LastError()!=AnimationStateMachineError::TransitionInProgress)return 1;
