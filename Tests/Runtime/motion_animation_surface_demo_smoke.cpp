...
int main() {
    ...
    if(!RunMotionAnimationSurfaceDemo(config,receipt,error)||receipt.renderedFrames!=4U||receipt.presentedFrames!=4U||receipt.visiblePixels==0U||receipt.frameHash==0U||receipt.finalX<=0.0F||receipt.sampledAnimation!=0.0F||receipt.endedLocomoting)return 1;
