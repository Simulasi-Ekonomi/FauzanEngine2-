...
int main() {
    ...
    if (RunFarmSurfaceDemo({64, 64, 4, true, output}, receipt, error) == false || error != FarmSurfaceDemoError::None || receipt.renderedFrames != 4U || receipt.presentedFrames != 4U || receipt.frameHash == 0U || receipt.buildings != 1U || receipt.npcs != 5U || receipt.worldFrameHash==0U || receipt.hudFrameHash!=receipt.frameHash || receipt.worldFrameHash==receipt.hudFrameHash) return 1;
