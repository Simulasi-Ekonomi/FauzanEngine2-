...
int main() {
    ...
    if(!RuntimeSaveCodec::Serialize(save,saveBytes,error)) return fail("header-corrupt");saveBytes[5]=1U;if(RuntimeSaveCodec::Deserialize(saveBytes,loaded,error)||error!=RuntimePersistenceError::Corrupt||loaded.kind!=before.kind||loaded.revision!=before.revision||loaded.payload!=before.payload) return 1;
