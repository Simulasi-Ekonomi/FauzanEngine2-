...
int main() {
    ...
    const RuntimeSaveEnvelope before=loaded;saveBytes.pop_back();if(RuntimeSaveCodec::Deserialize(saveBytes,loaded,error)||error!=RuntimePersistenceError::Corrupt||loaded.kind!=before.kind||loaded.revision!=before.revision||loaded.payload!=before.payload) return 1;
    if(!RuntimeSaveCodec::Serialize(save,saveBytes,error)) return 1;saveBytes.push_back(0);if(RuntimeSaveCodec::Deserialize(saveBytes,loaded,error)||error!=RuntimePersistenceError::TrailingBytes||loaded.kind!=before.kind||loaded.revision!=before.revision||loaded.payload!=before.payload) return 1;
    if(!RuntimeSaveCodec::Serialize(save,saveBytes,error)) return 1;saveBytes[5]=1U;if(RuntimeSaveCodec::Deserialize(saveBytes,loaded,error)||error!=RuntimePersistenceError::Corrupt||loaded.kind!=before.kind||loaded.revision!=before.revision||loaded.payload!=before.payload) return 1;
    RuntimeSaveEnvelope sensitive{"farm-world",8,{'t','o','k','e','n'}};if(RuntimeSaveCodec::Serialize(sensitive,saveBytes,error)||error!=RuntimePersistenceError::SensitiveContent) return 1;
