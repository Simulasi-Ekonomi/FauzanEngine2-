...
int main() {
    ...
    std::vector<MeshImportReceipt> setReceipts{{"sentinel",1U,1U,1U,false}}; const std::vector<MeshImportRequest> set{{"set.a",{},bytes(quad),{}},{"set.b",{},bytes(quad),{true}}};
    if(!pipeline.ImportObjSet(registry,meshes,set,setReceipts)||pipeline.LastError()!=MeshImportPipelineError::None||setReceipts.size()!=2U||setReceipts[0].assetId!="set.a"||setReceipts[1].assetId!="set.b"||registry.All().size()!=assets+2U||meshes.ResourceCount()!=staged+2U)return 1;
    const std::vector<MeshImportReceipt> preservedSet=setReceipts; const size_t setAssets=registry.All().size(),setStaged=meshes.ResourceCount(); const std::vector<MeshImportRequest> brokenSet{{"set.c",{},bytes(quad),{}},{"set.d",{},bytes(invalid),{}}};
    if(pipeline.ImportObjSet(registry,meshes,brokenSet,setReceipts)||pipeline.LastError()!=MeshImportPipelineError::StageFailed||registry.All().size()!=setAssets||meshes.ResourceCount()!=setStaged||setReceipts.size()!=preservedSet.size()||setReceipts[0].assetId!=preservedSet[0].assetId||pipeline.ImportObjSet(registry,meshes,{},setReceipts)||pipeline.LastError()!=MeshImportPipelineError::InvalidRequest||setReceipts.size()!=preservedSet.size())return 1;
    std::vector<MeshImportRequest> oversized(MeshImportPipeline::kMaxImportSetEntries+1U); if(pipeline.ImportObjSet(registry,meshes,oversized,setReceipts)||pipeline.LastError()!=MeshImportPipelineError::BatchCapacity||registry.All().size()!=setAssets||meshes.ResourceCount()!=setStaged||setReceipts.size()!=preservedSet.size())return 1;
    std::printf("MESH_IMPORT_PIPELINE_SMOKE_OK registry=1 ready=1 staged=1 atomic=1 batch=2 hash=%llu\n",static_cast<unsigned long long>(receipt.contentHash)); return 0;
}
