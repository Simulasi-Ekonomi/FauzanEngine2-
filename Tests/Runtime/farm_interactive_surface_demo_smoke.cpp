...
int main() {
    ...
    if (!RunFarmInteractiveSurfaceDemo({128U,96U,true,output},receipt,error) || error != FarmInteractiveSurfaceDemoError::None || receipt.frames != 17U || receipt.presentedFrames != 17U || receipt.worldFramebufferHash == 0U || receipt.hudFramebufferHash == receipt.worldFramebufferHash || receipt.selectedAction != FarmPlayerAction::Harvest || receipt.selectedActionMask != 0xFU || receipt.telemetry.growingTiles != 0U || receipt.telemetry.harvestableTiles != 0U || receipt.telemetry.questHarvestProgress != 1U || receipt.inventory.wheatSeeds != 31U || receipt.inventory.wheatProduce != 2U) return 1;
