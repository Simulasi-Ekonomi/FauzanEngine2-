...
int main() {
    ...
    if (!renderer.Initialize(96, 96) || !BindInput(input) || !session.Initialize(farm, world, set, assets, textures, renderer) || session.LastFrameReceipt().frame!=0U || !input.Push(kRight, true) || !session.Frame(input) || session.FrameCount() != 1U || session.LastFrameReceipt().frame!=1U || session.LastFrameReceipt().framebufferHash!=renderer.FrameHash() || session.LastFrameReceipt().telemetry.simulationTick!=farm.SimulationTick() || world.Character().x != 2U || world.Character().z != 1U) return 1;
