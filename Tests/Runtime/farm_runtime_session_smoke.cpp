...
int main() {
    ...
    if (!renderer.Initialize(96, 96) || !BindInput(input) || !session.Initialize(farm, world, set, assets, textures, renderer) || session.LastFrameReceipt().frame!=0U || !input.Push(kRight, true) || !session.Frame(input) || session.FrameCount() != 1U || session.LastFrameReceipt().frame!=1U || session.LastFrameReceipt().framebufferHash!=renderer.FrameHash() || session.LastFrameReceipt().telemetry.simulationTick!=farm.SimulationTick() || session.LastFrameReceipt().input.kind!=FarmPlayerInputKind::Movement || session.LastFrameReceipt().input.x!=2U || session.LastFrameReceipt().input.z!=1U || !session.LastFrameReceipt().availability.till || session.LastFrameReceipt().availability.plantWheat || session.LastFrameReceipt().availability.water || session.LastFrameReceipt().availability.harvest || world.Character().x != 2U || world.Character().z != 1U) return 1;
    const uint64_t afterMove = renderer.FrameHash();
    if (!input.Push(kRight, false) || !session.Frame(input) || !input.Push(kInteract, true) || !session.Frame(input) || farm.TileStateAt(2, 1) != FarmTileState::Tilled || session.FrameCount() != 3U || session.LastFrameReceipt().input.kind!=FarmPlayerInputKind::Action || session.LastFrameReceipt().input.action!=FarmPlayerAction::Till || session.LastFrameReceipt().input.x!=2U || session.LastFrameReceipt().input.z!=1U || session.LastFrameReceipt().availability.till || !session.LastFrameReceipt().availability.plantWheat || session.LastFrameReceipt().availability.water || session.LastFrameReceipt().availability.harvest) return 1;
