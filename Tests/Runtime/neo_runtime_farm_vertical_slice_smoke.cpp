...
int main() {
    ...
    if (!runtime.Initialize(config) || runtime.Farm() == nullptr || runtime.FarmWorld() == nullptr || runtime.Input() == nullptr ||
        runtime.FarmPlayerInput() == nullptr || runtime.SurfacePresenter() == nullptr || !runtime.SurfacePresenter()->IsReady()) return 1;
    ...
    const auto selectAndInteract = [&](FarmPlayerAction expected, FarmTileState expectedBefore, FarmTileState expectedAfter) {
        ...
        if (!runtime.RenderFarm() || !runtime.RouteFarmHudKeyboard(UiKeyboardKey::TabForward, receipt) || receipt.selected ||
            ...
            runtime.Farm()->TileStateAt(0U, 0U) != expectedAfter || !runtime.Input()->Push(MakeInputCode(InputDeviceType::Keyboard, 24U), false) ||
            !runtime.Tick()) return false;
        return true;
    };
    ...
    ok = ok && runtime.RestoreFarmProgressCheckpoint(checkpoint, restoredRevision) && restoredRevision == 42U &&
         runtime.FarmWorld()->Serialize() == savedWorld && runtime.LastFrameReceipt() == nullptr && runtime.LastFarmRenderReceipt() == nullptr && runtime.RenderFarm() &&
    ...
    ok = ok && !runtime.RestoreFarmProgressCheckpoint(corrupt, ignoredRevision) && runtime.LastError() == RuntimeError::CheckpointDecodeFailed &&
         ...
         runtime.LastFarmRenderReceipt()->hudFramebufferHash == preservedRender.hudFramebufferHash && runtime.Shutdown();
    if (!ok) {
        std::fprintf(stderr, "NEO_RUNTIME_FARM_VERTICAL_SLICE_SMOKE_FAIL\n");
        return 1;
    }
    std::printf("NEO_RUNTIME_FARM_VERTICAL_SLICE_SMOKE_OK actions=till,plant,water,harvest hud=1 cpu_present=1 checkpoint=atomic\n");
    return 0;
}
