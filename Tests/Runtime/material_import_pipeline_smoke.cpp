...
int main() {
    ...
    const std::vector<uint8_t> setValid{'n','e','w','m','t','l',' ','b','a','r','k','\n','K','d',' ','0','.','2',' ','0','.','4',' ','0','.','1','\n','n','e','w','m','t','l',' ','l','e','a','f','\n','K','d',' ','0','.','1',' ','0','.','7',' ','0','.','2','\n'}; std::vector<MaterialImportReceipt> receipts;
    if(!pipeline.ImportMtlSet(registry,materials,"foliage.material",{},setValid,{"bark","leaf"},receipts)||receipts.size()!=2U||receipts[0].materialName!="bark"||receipts[1].materialName!="leaf"||materials.Find("foliage.material","bark")==nullptr||materials.Find("foliage.material","leaf")==nullptr)return 1;
    const size_t setAssets=registry.All().size(),setMaterials=materials.ResourceCount();const std::vector<MaterialImportReceipt> setPreserved=receipts;
    if(pipeline.ImportMtlSet(registry,materials,"duplicate.material",{},setValid,{"bark","bark"},receipts)||pipeline.LastError()!=MaterialImportPipelineError::InvalidRequest||registry.All().size()!=setAssets||materials.ResourceCount()!=setMaterials||receipts.size()!=setPreserved.size())return 1;
    if(pipeline.ImportMtlSet(registry,materials,"missing.material",{},setValid,{"bark","missing"},receipts)||pipeline.LastError()!=MaterialImportPipelineError::StageFailed||registry.All().size()!=setAssets||materials.ResourceCount()!=setMaterials||receipts[0].materialName!=setPreserved[0].materialName)return 1;
    std::printf("MATERIAL_IMPORT_PIPELINE_SMOKE_OK assets=%zu materials=%zu set=2 hash=%llu atomic=1\n", registry.All().size(), materials.ResourceCount(), static_cast<unsigned long long>(preserved.contentHash)); return 0;
}
