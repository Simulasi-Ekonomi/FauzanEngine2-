...
int main() {
    ...
    if (!core.Initialize(base) || !core.Tick() || !core.RenderFarm() || core.Renderer() == nullptr || core.Renderer()->FrameHash() == 0U || core.LastFarmRenderReceipt() == nullptr || core.LastFarmRenderReceipt()->frame != 1U || core.LastFarmRenderReceipt()->worldFramebufferHash != core.Renderer()->FrameHash() || core.LastFarmRenderReceipt()->hudFramebufferHash != 0U || core.LastFarmRenderReceipt()->telemetry.simulationTick != 1U) return 1;
    ...
    if (!hud.RenderFarm() || hud.Renderer()->FrameHash() == 0U || hud.LastFarmRenderReceipt() == nullptr || hud.LastFarmRenderReceipt()->frame != 2U || !core.Shutdown() || !hud.Shutdown()) return 1;
