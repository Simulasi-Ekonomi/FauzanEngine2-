...
int main() {
    ...
    if (client.AuthoritativeState(999U, preservedAuthoritative) || client.LastError() != ReplicationError::UnknownEntity || preservedAuthoritative.networkId != 77U || preservedAuthoritative.ownerId != 88U || preservedAuthoritative.stateRevision != 99U || preservedAuthoritative.transform.x != 7.0F || !client.AuthoritativeState(100U, preservedAuthoritative) || client.LastError() != ReplicationError::None || preservedAuthoritative.networkId != 100U) return 3;
    ...
    if (ReplicationAcknowledgementCodec::Deserialize(acknowledgementBytes, decodedAcknowledgement, codecError) || codecError != ReplicationError::CorruptAcknowledgement || server.AcknowledgedSequence() != 1U) return 13;
    ...
    ReplicationSnapshot sameRevisionConflict = snapshot;
    sameRevisionConflict.sequence = 2U;
    sameRevisionConflict.serverTick = 2U;
    sameRevisionConflict.states[0].transform.x = 99.0F;
    ReplicationApplyReceipt preservedConflictReceipt{81U, 82U, 83U, 84U, 85U, 86U, 87U, true};
    if (!ReplicationSnapshotCodec::Serialize(sameRevisionConflict, encoded, codecError) || !ReplicationSnapshotCodec::Deserialize(encoded, decoded, codecError) || client.ApplyServerSnapshot(decoded, preservedConflictReceipt) || client.LastError() != ReplicationError::StaleSnapshot || preservedConflictReceipt.sequence != 81U || preservedConflictReceipt.serverTick != 82U || preservedConflictReceipt.appliedEntities != 83U || preservedConflictReceipt.spawnedEntities != 84U || preservedConflictReceipt.despawnedEntities != 85U || preservedConflictReceipt.interpolatedEntities != 86U || preservedConflictReceipt.reconciledPredictions != 87U || !preservedConflictReceipt.accepted) return 20;
    localAfterReconcile = clientScene.GetTransform(clientLocal);
    if (localAfterReconcile == nullptr || std::abs(localAfterReconcile->x - 6.0F) > 0.0001F) return 20;
    ...
    ownershipTransfer.states[0].stateRevision = 2U;
    ...
    ownershipTransfer.states[0].stateRevision = 3U;
