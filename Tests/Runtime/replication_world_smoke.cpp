...
int main() {
    ...
    if (!ReplicationSnapshotCodec::Serialize(chronologySnapshot, chronologyBytes, codecError) || !ReplicationSnapshotCodec::Deserialize(chronologyBytes, decoded, codecError) || client.ApplyServerSnapshot(decoded, apply) || client.LastError() != ReplicationError::StaleSnapshot || client.SnapshotSequence() != 1U) return 13;
    ...
    if (!ReplicationSnapshotCodec::Serialize(unknownEntitySnapshot, unknownEntityBytes, codecError) || !ReplicationSnapshotCodec::Deserialize(unknownEntityBytes, decoded, codecError) || client.ApplyServerSnapshot(decoded, apply) || client.LastError() != ReplicationError::UnknownEntity) return 14;
    localAfterReconcile = clientScene.GetTransform(clientLocal);
    if (localAfterReconcile == nullptr || std::abs(localAfterReconcile->x - 6.0F) > 0.0001F) return 15;
    if (!client.SetInterpolationAlphaPermille(500U) || !client.ApplyInterpolation(apply) || apply.interpolatedEntities != 1U) return 16;
    const Transform3* remoteAfterInterpolation = clientScene.GetTransform(clientRemote);
    if (remoteAfterInterpolation == nullptr || std::abs(remoteAfterInterpolation->x - 15.0F) > 0.0001F) return 17;
    if (client.ApplyServerSnapshot(snapshot, apply) || client.LastError() != ReplicationError::StaleSnapshot) return 18;
