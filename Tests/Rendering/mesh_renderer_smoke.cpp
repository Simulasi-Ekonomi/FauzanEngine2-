...
int main() {
    ...
    std::vector<MeshVertex> partialFailure=triangle;partialFailure.insert(partialFailure.end(),{{{100.0F,-0.5F,2.0F},{0,0,1}},{{101.0F,-0.5F,2.0F},{0,0,1}},{{100.0F,0.6F,2.0F},{0,0,1}}});const std::vector<uint16_t> partialIndices{0,1,2,3,4,5};
    if(!renderer.Clear(0xFF123456U)){return 1;}const uint64_t partialPrior=renderer.FrameHash();if(mesh.Draw(partialFailure,partialIndices,{},{0xFFFFFFFF,1.0F,0.0F},{{0,0,1}},camera,renderer)||mesh.LastError()!=MeshRenderError::ProjectionFailed||renderer.FrameHash()!=partialPrior){std::printf("MESH_RENDERER_SMOKE_FAIL stage=atomic-reject error=%u\n",static_cast<unsigned>(mesh.LastError()));return 1;}
    ...
    std::printf("MESH_RENDERER_SMOKE_OK triangles=2 perspective=1 orientedCamera=1 texture=ppm+bmp tint=1 intensity=1 atomicReject=1 lights=2 depth=1 hash=%llu\n",static_cast<unsigned long long>(hash)); return 0;
}
