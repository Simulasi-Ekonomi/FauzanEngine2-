...
int main() {
    ...
    SpriteDraw alphaFront{0,0,1,1,1,0,0,0x800000FFU}; alphaFront.depthWrite=false; const SpriteDraw opaqueBack{0,0,1.5F,1,1,0,1,0xFFFF0000U}; batch.Clear(); if(!renderer.Clear(0xFF000000U)||!batch.Queue(alphaFront)||!batch.Queue(opaqueBack)||!batch.Flush(renderer,camera)||renderer.PixelAt(32,32)!=0xFFFF0000U)return 1; alphaFront.depthWrite=true; batch.Clear(); if(!renderer.Clear(0xFF000000U)||!batch.Queue(alphaFront)||!batch.Queue(opaqueBack)||!batch.Flush(renderer,camera)||renderer.PixelAt(32,32)==0xFFFF0000U)return 1;
