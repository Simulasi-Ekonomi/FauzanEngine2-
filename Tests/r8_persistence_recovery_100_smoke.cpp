...
int main() {
    ...
    const std::filesystem::path rootDir = std::filesystem::temp_directory_path() / "r8_save_root";
    ...
    if (!NeoEngine::AtomicSaveFile::Write(rootDir, slotName, payload, err)) return 1;
    std::vector<uint8_t> readData;
    if (!NeoEngine::AtomicSaveFile::Read(rootDir, slotName, readData, err) || readData != payload) return 1;
