...
|---|---|---|---|---|
| R1 | Canonical game tool | A typed, versioned game-tool contract for world, rules, content, save migration, invalid input, and deterministic replay. | FarmWorldTool and RPG sandbox cover bounded local state only. | Not passed |
| R2 | Complete game loop | A playable vertical slice implementing onboarding, core loop, progression, failure/recovery, balancing data, content authoring, and player-facing UX. | Headless Farm/RPG state and CPU frames only. | Not passed |
